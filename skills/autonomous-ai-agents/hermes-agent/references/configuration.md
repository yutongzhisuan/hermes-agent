...
Edit with `xhermes config edit` or `xhermes config set section.key value`.
Full reference: https://xhermes-agent.nousresearch.com/docs/user-guide/configuration
...
`xhermes config check` reports sections missing from an older config.
