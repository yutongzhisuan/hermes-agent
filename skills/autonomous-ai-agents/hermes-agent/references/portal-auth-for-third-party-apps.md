...
## Layer 1 — Is this thing a XHermes plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the XHermes process**. Its LLM calls go through XHermes's already-configured provider. | Already uses Portal if user's XHermes is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside XHermes. That LLM call is already authenticated
via XHermes's provider config. The plugin's own server auth (e.g.
...
What XHermes Agent has that *feels* like OAuth — `xhermes login --provider nous`
opening a browser, user signs in, token lands in `~/.xhermes/auth.json` — is a
**XHermes-specific browser flow**. Under the hood it produces a credential
XHermes uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read XHermes's existing Portal credential out of `~/.xhermes/auth.json`.
...
rides on the credential XHermes already holds.

Where this could live in XHermes:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for XHermes itself.
...
1. First decide: XHermes plugin (runs inside XHermes) or separate app? If plugin,
   it already uses Portal via XHermes's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   XHermes agent itself") is fully supported. XHermes treats agent
...
pitch in the Tool Gateway announcement is specifically about XHermes Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when XHermes is configured to use Portal as its provider. It is
