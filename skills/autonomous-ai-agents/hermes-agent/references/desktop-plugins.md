...
Write plugins for the XHermes desktop app: statusbar items, layout panes,
...
system (`~/.xhermes/plugins/`) is otherwise documented separately.
...
- The XHermes desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$XHERMES_HOME/desktop-plugins/` (usually
  `~/.xhermes/desktop-plugins/`).

## How to Run

1. Create `$XHERMES_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.xhermes/...` by default, or `~/.xhermes/profiles/<profile>/...` under a
...
The ONLY import surface is `@xhermes/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.xhermes/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@xhermes/plugin-sdk`, `react`, and
