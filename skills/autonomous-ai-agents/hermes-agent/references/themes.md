# Themes / Skins — Author a XHermes Color Theme

Author a XHermes **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.xhermes/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for XHermes itself.
...
3. `write_file` it to `<xhermes-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `xhermes config set display.skin default` (or they can `/skin default`).
...
```
xhermes skin set <key> <hex>      # e.g. xhermes skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `xhermes skin set background "#08201f"` changes
only the background; `xhermes skin use <name>` / `xhermes skin list` switch and
enumerate.
...
- **Never hand-edit `config.yaml` to activate.** Use `xhermes config set
...
- **You apply it, not the user.** `xhermes config set display.skin <name>` is
...
- `read_file` the written `<xhermes-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `xhermes config get display.skin` and confirm it reports `<name>`.
