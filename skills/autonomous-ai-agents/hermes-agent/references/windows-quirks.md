...
XHermes runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`xhermes config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually XHermes's env scrubber dropping
...
XHermes-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every XHermes tool and most Windows APIs. Prefer forward slashes in code
