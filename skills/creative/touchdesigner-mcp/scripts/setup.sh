...
XHERMES_HOME_DIR="${XHERMES_HOME:-$HOME/.xhermes}"
XHERMES_CFG="${XHERMES_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure XHermes config has twozero_td MCP entry ──
if [[ ! -f "$XHERMES_CFG" ]]; then
    echo -e " ${FAIL} XHermes config not found at ${XHERMES_CFG}"
    manual_steps+=("Create ${XHERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$XHERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in XHermes config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to XHermes config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$XHERMES_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${XHERMES_CFG} manually"); }
    manual_steps+=("Restart XHermes session to pick up config change")
fi
