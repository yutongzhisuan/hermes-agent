...
## 5. XHermes Configuration

### Config location

`$XHERMES_HOME/config.yaml` (defaults to `~/.xhermes/config.yaml` when `XHERMES_HOME` is unset)
...
Restart the XHermes session for changes to take effect. The MCP connection is
