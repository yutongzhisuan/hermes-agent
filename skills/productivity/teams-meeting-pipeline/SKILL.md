...
Everything operator-facing is a `xhermes teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${XHERMES_HOME:-~/.xhermes}/.env`:
...
```bash
xhermes teams-pipeline validate              # config snapshot — run first after any change
xhermes teams-pipeline token-health          # Graph token status
xhermes teams-pipeline token-health --force-refresh   # force a fresh token acquisition
xhermes teams-pipeline list                  # recent meeting jobs
xhermes teams-pipeline list --status failed  # only failed jobs
xhermes teams-pipeline show <job-id>         # full detail of one job
xhermes teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
xhermes teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
xhermes teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
xhermes teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```
...
xhermes teams-pipeline maintain-subscriptions            # renew near-expiry ones
xhermes teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `xhermes teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `xhermes cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
