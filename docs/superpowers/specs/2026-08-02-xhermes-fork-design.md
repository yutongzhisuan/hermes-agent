...
- **日期**: 2026-08-02
- **状态**: 待审核（已按评审修订）
- **基准**: xhermes-agent v0.19.1
- **目标**: fork xhermes-agent 为 xhermes-agent，与原有 xhermes-agent 同机共存、互不干扰，且保留上游同步能力

## 1. 背景与目标

### 1.1 目标

1. 从 xhermes-agent fork 出自己的 agent（xhermes-agent）
2. 打包命名、安装命名、目录命名与 xhermes-agent **完全错开**
3. 与原有 xhermes-agent **同机共存、互不干扰**
4. **保留上游同步能力**：定期拉取 xhermes-agent 的修复与新功能
...
| 存量迁移 | **禁止静默 rename** | rename `~/.xhermes`→`~/.xhermes` 会破坏已装 xhermes，与共存目标冲突 |
...
| 概念 | xhermes | xhermes |
|---|---|---|
| PyPI / 发行名 | `xhermes-agent` | `xhermes-agent` |
| CLI | `xhermes` | `xhermes` |
| 家目录 (POSIX) | `~/.xhermes` | `~/.xhermes` |
| 家目录 (Windows) | `%LOCALAPPDATA%\xhermes` | `%LOCALAPPDATA%\xhermes` |
| 代码安装子目录 | `$HOME/xhermes-agent` | `$HOME/xhermes-agent` |
| 完整默认安装树 | `~/.xhermes/xhermes-agent` | `~/.xhermes/xhermes-agent` |
| systemd unit 基名 | `xhermes-gateway` | `xhermes-gateway` |
| launchd label | `ai.xhermes.gateway` | `ai.xhermes.gateway` |
| Electron appId | `com.nousresearch.xhermes` | `com.<you>.xhermes` |
...
xhermes 与 xhermes 的隔离依赖 **4 个正交维度**，缺一不可：

| 维度 | xhermes | xhermes | 隔离机制 |
|---|---|---|---|
| Python 环境 | 自己的 venv | **独立 venv** | site-packages 永不相交 |
| 家目录 | `~/.xhermes` | `~/.xhermes` | 所有状态文件隔离 |
| 命令行 | `xhermes` | `xhermes` | PATH 无冲突 |
| 进程/服务名 | `xhermes-*` | `xhermes-*` | 互不误杀 / 互不抢 unit |
...
**禁止**：`pip install` 装进 xhermes 的 venv / 系统 site-packages。

**效果**：命令行 `xhermes` 指向 xhermes venv 的 bin，`xhermes` 指向 xhermes venv 的 bin，各自装各自的包，永不相交。
...
| 位置 | 原值 | 改为 |
|---|---|---|
| `pyproject.toml [project] name` | `xhermes-agent` | `xhermes-agent` |
| `pyproject.toml [project.scripts]` | `xhermes = hermes_cli.main:main` | `xhermes = hermes_cli.main:main` |
| `pyproject.toml [project.scripts]` | `xhermes-agent = run_agent:main` | `xhermes-agent = run_agent:main` |
| `pyproject.toml [project.scripts]` | `xhermes-acp = acp_adapter.entry:main` | `xhermes-acp = acp_adapter.entry:main` |
| 自引用 extras（termux/termux-all/all） | `xhermes-agent[cron]` 等 | `xhermes-agent[...]` |
...
def _get_platform_default_hermes_home() -> Path:
    if sys.platform == "win32":
        ...
        return base / "xhermes"           # 原 "xhermes"
    return Path.home() / ".xhermes"       # 原 ".xhermes"
```
...
3. `~/.xhermes`.parent.name 是用户名而非 `"profiles"` → 不走 profile 分支
4. **返回 `~/.xhermes`** → xhermes 把 xhermes 的 home 当作自身 root
...
**范围说明**：仓库内字符串/`~/.xhermes` 出现点远多于"十几处"。实施时以**运行时路径解析**为准，用搜索清单驱动，不依赖固定行号。至少覆盖：

| 类别 | 代表位置 | 改法 |
|---|---|---|
| CLI 早期 fallback | `hermes_cli/main.py`（config / profiles / desktop-ssh 等） | `.xhermes` → `.xhermes` 或 `get_hermes_home()` |
| Auth seatbelt | `hermes_cli/auth.py`（pytest 防护写死 `~/.xhermes/auth.json`） | 改为 `~/.xhermes`（测试防护目标随 fork 变） |
| 文件安全 / 沙箱路径段 | `agent/file_safety.py`（镜像路径中的 `.xhermes` 段） | 段名改为 `.xhermes`，并与沙箱 home 布局一致 |
| 网关 / TUI / MCP | `gateway/run.py`、`tui_gateway/server.py`、`mcp_serve.py` | `get_hermes_home()` |
| Node bridge | WhatsApp bridge 等 JS 中 `path.join(HOME, '.xhermes', ...)` | `.xhermes` |
| 插件 / optional-skills | 各硬编码 fallback | 同模式 |
| 项目插件目录 | `hermes_cli/plugins.py` 扫描 `./.xhermes/plugins/` | 改为 `./.xhermes/plugins/`（避免与同 cwd 的 xhermes 项目插件树共享） |
...
**禁止**在启动路径里 `Path.rename(~/.xhermes → ~/.xhermes)`——同机已装 xhermes 时会直接搬走对方数据，与共存目标冲突。

| 方案 | 行为 | 何时用 |
|---|---|---|
| **默认** | 始终使用空的 `~/.xhermes`；不碰 `~/.xhermes` | 同机共存 / 全新安装 |
| **可选** | `xhermes migrate --from-xhermes`：**copy**（非 rename）选定子集到 `~/.xhermes` | 用户明确要从 xhermes 拷配置/技能 |
| **禁止** | 静默 rename / 无确认覆盖 | — |
...
| `gateway/status.py` basename / `_GATEWAY_KIND` | `xhermes`→`xhermes`、`xhermes-gateway`→`xhermes-gateway`；kind 同改 |
| `hermes_cli/gateway.py` `_SERVICE_BASE` | `"xhermes-gateway"` → `"xhermes-gateway"`（`get_service_name()` 中枢） |
| `hermes_cli/gateway.py` launchd | `ai.xhermes.gateway` → `ai.xhermes.gateway`（含 profile 后缀形态） |
| `hermes_cli/gateway.py` tmux session 名 | `xhermes` → `xhermes` |
...
| `hermes_cli/main.py`（exe 更新隔离） | `xhermes.exe` → `xhermes.exe` 等 |
| 用户提示串 | `"xhermes gateway restart"` → `"xhermes gateway restart"` 等 |
| **`scripts/xhermes-gateway`（独立服务脚本）** | `SERVICE_NAME="xhermes-gateway"`→`"xhermes-gateway"`；plist 路径 `ai.xhermes.gateway.plist`→`ai.xhermes.gateway.plist`；plist 内 `<string>ai.xhermes.gateway</string>`→`ai.xhermes.gateway`；所有 `launchctl ... ai.xhermes.gateway` 命令；**venv 路径 `PROJECT_DIR/venv`→`~/.xhermes/xhermes-agent/venv`**；或标记为 fork 不发行（若 xhermes 统一走 `xhermes gateway` CLI 路径） |

> **`ai.xhermes.gateway` 标签额外出现点**（§3.3 表自称"全集"，以下必须覆盖，否则 launchctl 检测/防护失配；§5.4 checklist regex `ai\.xhermes\.` 可在合并后兜底扫描，但实施时应前置覆盖）：
...
**问题**：同名 profile（如 `coder`）会互相覆盖 `~/.local/bin/coder`；wrapper 内容硬编码 `xhermes` 会错调对方。
...
安装/文档提示用户把 `~/.xhermes/bin` 加入 PATH（可排在 xhermes 之后或之前，互不影响文件名空间）。
...
| 位置 | 改动 | 优先级 |
|---|---|---|
| `ui-tui/package.json` | `xhermes-tui` → `xhermes-tui` | 高 |
| `ui-tui/packages/xhermes-ink/package.json` | `@xhermes/ink` → `@xhermes/ink` | 高 |
| `apps/shared/package.json` | `@xhermes/shared` → `@xhermes/shared` | 高（先改它再改依赖方） |
| `apps/bootstrap-installer/package.json` | `@xhermes/bootstrap-installer` → `@xhermes/bootstrap-installer` | 中 |
...
```text
OFFICIAL_REPO_URL = "https://github.com/NousResearch/xhermes-agent.git"
```
...
| `upstream` | `NousResearch/xhermes-agent`（只读；**不**作为 `update` 的默认 pull 源） |
...
| 项 | 结论 |
|---|---|
| `~/.xhermes` vs `~/.xhermes` | ✅ 状态文件全隔离（在**禁止静默 rename**前提下） |
...
```
upstream/main  (NousResearch/xhermes-agent)  ── 只读，定期 fetch
   │
...
1. `rg -n 'shutil\.which\("xhermes"\)|_SERVICE_BASE = "xhermes|SERVICE_NAME = "xhermes|ai\.xhermes\.|~/\.xhermes/xhermes-agent|OFFICIAL_REPO_URL.*NousResearch'` 在**产品路径**上应无残留（测试里的历史字符串另议）
...
| Phase 2 | 家目录/env；**无静默迁移** | `get_hermes_home()==~/.xhermes`；存在 `~/.xhermes` 时不被改动 |
| Phase 3 | 进程/服务/wrapper/端口 | unit 名、which、端口双开 |
| Phase 4 | 安装/桌面/Docker | 安装树 `~/.xhermes/xhermes-agent`；桌面启动 |
| Phase 5 | 前端/品牌/appId | build 通过；与 XHermes.app 可并存 |
...
1. `xhermes --version` 与 `xhermes --version` 各自正常
...
5. 同机已有 `~/.xhermes` 时，启动 xhermes **不 rename/不删除**该目录
6. `xhermes` 与 `xhermes` 同时跑 gateway/serve：默认端口与 systemd/launchd unit 互不冲突
7. profile wrapper 落在 `~/.xhermes/bin`，内容调用 `xhermes`
8. 桌面端 appId / AppUserModelId 与 XHermes 不同，可并列安装
...
test -d "$HOME/.xhermes" && test -d "$HOME/.xhermes"
...
| 2026-08-02 | 二次修订（代码核查驱动）：补入 `scripts/xhermes-gateway` 独立脚本（硬编码 `SERVICE_NAME`/plist/label/venv 路径）至 §3.3 改动全集；补入 `get_default_hermes_root()` 绕过 `XHERMES_HOME` 的修复要求（§3.2b-1）；补全 `ai.xhermes.gateway` 标签 5 个额外出现点；§5.4 checklist regex 增 `SERVICE_NAME` 模式；§7 风险清单增 2 条 |
