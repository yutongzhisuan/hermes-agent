...
Supersedes: PR #31781 (prompt_toolkit `xhermes profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `xhermes profile wizard` in the terminal.
...
needs, and a profile is just a XHERMES_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.xhermes/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — XHERMES_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with XHERMES_HOME override |
...
### Seam #1 — hub-skill install cannot use the XHERMES_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = XHERMES_HOME / "skills"` at **module
...
A fresh subprocess re-imports `skills_hub` with the profile's XHERMES_HOME bound
...
2. write model (synchronous, XHERMES_HOME override)
3. write selected MCP servers (synchronous, XHERMES_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `xhermes -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, XHERMES_HOME override
...
page afterward. Mirror `_write_profile_model`'s XHERMES_HOME-override helper for
...
- Backend E2E with isolated XHERMES_HOME: POST a full create body
