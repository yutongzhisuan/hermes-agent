...
XHermes includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into XHermes and does not require `xhermes plugins enable
observability/nemo_relay`. XHermes remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
XHermes execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `xhermes-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

XHermes requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
XHermes installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the XHermes process so configured interceptors can
...
Collection remains off unless XHermes policy enables it:
...
XHermes core owns one Relay host and one isolated Relay session scope per XHermes
...
marks do not require XHermes plugin registration. Shared-metrics marks must
...
```text
XHermes turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> XHermes shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

XHermes sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `xhermes.task_run`, parented to
the owning XHermes session. The start counter contains only bounded execution
...
provider attempts for the same XHermes API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their XHermes tool-call
...
cancellations. Active task ownership follows the task ID if XHermes rotates its
...
Run a real XHermes CLI turn against the deterministic local model server:
