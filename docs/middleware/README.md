...
XHermes middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing XHermes'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before XHermes calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving XHermes retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving XHermes guardrails,
...
- `telemetry_schema_version`: currently `xhermes.observer.v1`
- `middleware_schema_version`: currently `xhermes.middleware.v1`
...
XHermes stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, XHermes runs
them as a nested chain in registration order. Middleware failures are fail-open:
XHermes logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, XHermes applies middleware in this order:
...
For each tool call, XHermes applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal XHermes pre-execution path against the effective arguments:
...
```bash
uv sync
uv run xhermes plugins enable <plugin-name>
uv run xhermes chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/xhermes-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape XHermes expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, XHermes treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, XHermes preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. XHermes does not convert downstream
