...
description: "How XHermes builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

XHermes deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.xhermes.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.xhermes/SOUL.md)
You are XHermes, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance XHermes
...
`SOUL.md` lives at `~/.xhermes/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
- **YAML frontmatter stripped** — `.xhermes.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, XHermes concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs XHermes already loads, rather than editing Python templates in place.
...
- if you want to change how XHermes assembles prompts for everyone, change Python and treat it as a code contribution
