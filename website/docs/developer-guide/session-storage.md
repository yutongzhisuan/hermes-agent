...
XHermes Agent uses a SQLite database (`~/.xhermes/state.db`) to persist session
...
Multiple xhermes processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.xhermes/state.db
