...
title: "Desktop Plugin SDK (@xhermes/plugin-sdk)"
description: "Extend the native XHermes Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [XHermes Desktop](/user-guide/desktop) app is contribution-driven: every
...
It imports one module — `@xhermes/plugin-sdk` — and gets everything: the app's
...
```javascript
// ~/.xhermes/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@xhermes/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, XHermes' }),
      jsx('div', {
...
The only importable specifiers are `@xhermes/plugin-sdk`, `react`, and
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `xhermes.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@xhermes/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@xhermes/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@xhermes/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@xhermes/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@xhermes/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@xhermes/plugin-sdk'
...
```javascript
import { queryClient } from '@xhermes/plugin-sdk'
...
`dashboard/` subfolder of a regular XHermes plugin and declare it in a
`manifest.json`:
...
xhermes-agent codebase directly (`hermes_state`, `hermes_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.xhermes/`)
...
(`xhermes.plugin.<id>.*`) so plugins can't read or clobber each other:
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@xhermes/plugin-sdk` alias.
2. It's still lint-fenced to `@xhermes/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@xhermes/plugin-sdk`, `react`,
...
### Agents: the `xhermes-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`xhermes-desktop-plugins`** skill — it carries the same contract as this page in
...
`@xhermes/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.xhermes/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.xhermes/logs/errors.log` for
