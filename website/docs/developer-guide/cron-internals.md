...
description: "How XHermes stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `xhermes cron` subcommands |
...
Jobs are stored in `~/.xhermes/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `xhermes cron` commands are run or during active CLI sessions.
...
| Local file | `local` | Save to `~/.xhermes/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `xhermes cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `xhermes cron` CLI provides direct job management:

```bash
xhermes cron list                    # Show all jobs
xhermes cron create                  # Interactive job creation (alias: add)
xhermes cron edit <job_id>           # Edit job configuration
xhermes cron pause <job_id>          # Pause a running job
xhermes cron resume <job_id>         # Resume a paused job
xhermes cron run <job_id>            # Trigger immediate execution
xhermes cron remove <job_id>         # Delete a job
```
