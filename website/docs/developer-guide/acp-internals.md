...
The ACP adapter wraps XHermes' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
xhermes acp / xhermes-acp / python -m acp_adapter
...
- `allow_once` -> XHermes `once`
- `allow_always` -> XHermes `always`
- reject options -> XHermes `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps XHermes tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["xhermes-acp"])
...
Instead it reuses XHermes' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured XHermes provider/credentials. It also always advertises a terminal setup auth method (`xhermes-setup`, args `--setup`) so first-run ACP clients can open XHermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.xhermes/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `xhermes-acp` toolset definition
- `hermes_cli/main.py` — `xhermes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `xhermes-acp` script
