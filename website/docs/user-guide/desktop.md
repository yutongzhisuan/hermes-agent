...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `xhermes desktop --cwd <path>` (or the `HERMES_DESKTOP_CWD` environment variable).
...
| `--xhermes-root PATH` | Override the XHermes source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
