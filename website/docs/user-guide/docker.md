...
docker run -it --rm \
  -v ~/.hermes:/opt/data \
  yutongzhisuan/hermes-agent setup
```
...
docker run -d \
  ...
  -v ~/.hermes:/opt/data \
  -p 8642:8642 \
  yutongzhisuan/hermes-agent gateway run
```
...
services:
  hermes-work:
    image: yutongzhisuan/hermes-agent:latest
...
FROM yutongzhisuan/hermes-agent:latest
...
```sh
docker pull yutongzhisuan/hermes-agent:latest
docker rm -f hermes
docker run -d \
  --name hermes \
  --restart unless-stopped \
...
When a tool must be available immediately on every container start with no re-install delay, build a new image that inherits from `yutongzhisuan/hermes-agent` and installs the tool in a layer:
...
The entrypoint script and `/opt/data` semantics are inherited unchanged, so the rest of this page still applies. Remember to rebuild the image when pulling a newer upstream `yutongzhisuan/hermes-agent`.
...
docker run -it --rm yutongzhisuan/hermes-agent:latest version     # Verify version
