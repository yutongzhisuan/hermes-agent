...
XHermes Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, XHermes checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.xhermes/config.yaml`:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that XHermes refuses to run them **regardless** of:
...
Configure the timeout in `~/.xhermes/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.xhermes/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.xhermes/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` xhermes/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.xhermes/config.yaml`:
...
:::tip
Use `xhermes config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`xhermes approvals suggest`)
...
```bash
xhermes approvals suggest            # dry run — prints a numbered proposal
xhermes approvals suggest --apply 1,3  # merge picks into command_allowlist
xhermes approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.xhermes/state.db`) for
...
Do not ask the agent to `patch` `~/.xhermes/cron/jobs.json` directly. Use the `cronjob` tool, [`xhermes cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other XHermes control files when write safety blocks direct edits.
...
When running the messaging gateway, XHermes controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.xhermes/.env`:
...
For more flexible authorization, XHermes includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `xhermes pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.xhermes/config.yaml`:
...
:::tip Docker users: run pairing commands as the `xhermes` user
The official Docker image runs the gateway as the unprivileged `xhermes` user
...
Always pass `-u xhermes`:

```bash
docker exec -u xhermes xhermes-agent xhermes pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/NousResearch/xhermes-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.xhermes/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, XHermes applies strict security hardening to every container.
...
Container resources are configurable in `~/.xhermes/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.xhermes/sandboxes/docker/<task_id>/`
...
| **terminal** (local) | Blocks explicit XHermes infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- XHermes infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
XHermes integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run XHermes under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.xhermes/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.xhermes/logs/` for unauthorized access attempts
10. **Keep updated** — run `xhermes update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.xhermes/.env`:
...
XHermes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `xhermes doctor` for the full remediation.
- **`xhermes doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. XHermes installs these **lazily** on first use rather than eagerly under `xhermes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `xhermes tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `xhermes tools`.
