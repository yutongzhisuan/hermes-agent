...
`~/.xhermes/config.yaml`, `~/.xhermes/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/xhermes`:

```text
/etc/xhermes/
├── config.yaml     # managed config layer (wins over ~/.xhermes/config.yaml)
└── .env            # managed env layer (wins over ~/.xhermes/.env + shell)
```
...
The location can be relocated with the `XHERMES_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `XHERMES_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by XHermes.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export XHERMES_MANAGED_DIR=/opt/org/xhermes-policy
```

:::warning
A user who can set `XHERMES_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `xhermes doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/xhermes/config.yaml` (managed) | `/etc/xhermes/.env` (managed) |
| 2 | `~/.xhermes/config.yaml` (user) | `~/.xhermes/.env` (user) |
...
```bash
xhermes config        # shows a header naming the managed source + the pinned keys
xhermes doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, XHermes refuses and names the source:

```bash
$ xhermes config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/xhermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `xhermes config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/xhermes

# Pin some config values for every user on this machine
sudo tee /etc/xhermes/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/xhermes/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/xhermes
sudo chmod 0644 /etc/xhermes/config.yaml /etc/xhermes/.env
```

Changes take effect on the next XHermes start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`xhermes doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs XHermes as `root`), managed scope is advisory.
