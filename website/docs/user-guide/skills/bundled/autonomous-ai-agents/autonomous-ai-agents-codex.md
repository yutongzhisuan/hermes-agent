...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`xhermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-xhermes-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the XHermes terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For XHermes itself, `model.provider: openai-codex` uses XHermes-managed Codex
OAuth from `~/.xhermes/auth.json` after `xhermes auth add openai-codex`. For the
...
## XHermes Gateway Caveat

When invoking the Codex CLI from a XHermes gateway/service context (for example,
