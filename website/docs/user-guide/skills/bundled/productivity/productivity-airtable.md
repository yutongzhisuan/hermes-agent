...
4. Store the token in `${XHERMES_HOME:-~/.xhermes}/.env` (or via `xhermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${XHERMES_HOME:-~/.xhermes}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
