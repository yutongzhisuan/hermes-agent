---
title: "XHermes S6 Container Supervision — Modify or debug s6 services in the XHermes Docker image"
sidebar_label: "XHermes S6 Container Supervision"
description: "Modify or debug s6 services in the XHermes Docker image"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# XHermes S6 Container Supervision

Modify or debug s6 services in the XHermes Docker image.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `xhermes skills install official/devops/xhermes-s6-container-supervision` |
| Path | `optional-skills/devops/xhermes-s6-container-supervision` |
| Version | `1.0.0` |
...
# XHermes s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the XHermes Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/xhermes/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the XHermes Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-xhermes-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
└── CMD ("main program")               ← /opt/xhermes/docker/main-wrapper.sh
    └── routes user args: bare exec | xhermes subcommand | xhermes (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/xhermes/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-xhermes-setup. |
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `xhermes gateway start/stop/restart` and routes to s6 when running in a container. |
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/xhermes/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to xhermes via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main xhermes is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to xhermes via `s6-setuidgid xhermes` at the top of run (unless you specifically need root).
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `xhermes -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `xhermes profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `xhermes-agent-dev`: General xhermes-agent codebase navigation
- `xhermes-tool-quirks`: Specific XHermes-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with xhermes built-in tools.
