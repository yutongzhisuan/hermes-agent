...
| | |
|---|---|
| Source | Optional — install with `xhermes skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by XHermes Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during XHermes terminal calls (desktop app flow only)
...
### Service Account (recommended for XHermes)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${XHERMES_HOME:-~/.xhermes}/.env` (the skill will prompt for this on first load).
...
## XHermes Execution Pattern (desktop app flow)

XHermes terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/xhermes-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/xhermes-op.sock"
