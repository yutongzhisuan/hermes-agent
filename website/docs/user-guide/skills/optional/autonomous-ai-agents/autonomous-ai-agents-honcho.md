---
title: "Honcho — Configure and troubleshoot Honcho memory for XHermes"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for XHermes"
---
...
Configure and troubleshoot Honcho memory for XHermes.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `xhermes skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for XHermes

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every XHermes profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/xhermes#running-honcho-locally-with-xhermes
...
3. **AI peer card** -- the identity card for this XHermes profile's AI peer
...
Honcho models conversations as interactions between **peers**. XHermes creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this XHermes instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each XHermes run |
| `global` | Single session across all directories |

Manual override: `xhermes honcho map my-project-name`
...
Each XHermes profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
1. Creates a `xhermes.coder` host block in `honcho.json`
...
Guidelines for XHermes when Honcho memory is active.
...
Config file: `$XHERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `xhermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.xhermes/config.yaml`.

### Memory not persisting across sessions
Check `xhermes honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `xhermes profile create <name> --clone`. For existing profiles: `xhermes honcho sync`.
...
| Command | Description |
|---------|-------------|
| `xhermes honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `xhermes honcho status` | Show resolved config, connection test, peer info for active profile |
| `xhermes honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `xhermes honcho disable` | Disable Honcho for the active profile |
| `xhermes honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
...
| `xhermes honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `xhermes honcho sessions` | List known directory-to-session-name mappings |
| `xhermes honcho map <name>` | Map current working directory to a Honcho session name |
| `xhermes honcho identity` | Seed AI peer identity or show both peer representations |
| `xhermes honcho sync` | Create host blocks for all XHermes profiles that don't have one yet |
| `xhermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to XHermes + Honcho |
| `xhermes memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `xhermes memory status` | Show active memory provider and config |
| `xhermes memory off` | Disable external memory provider |
