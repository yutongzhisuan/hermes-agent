...
| | |
|---|---|
| Source | Optional — install with `xhermes skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), XHermes Agent |
| License | MIT |
...
The script reads `${XHERMES_HOME:-~/.xhermes}/.env` for two optional defaults:
...
Helper script: `~/.xhermes/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${XHERMES_HOME:-~/.xhermes}/.env`.
