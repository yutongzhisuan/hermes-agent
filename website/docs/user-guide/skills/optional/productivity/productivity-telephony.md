...
| | |
|---|---|
| Source | Optional — install with `xhermes skills install official/productivity/telephony` |
...
This optional skill gives XHermes practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${XHERMES_HOME:-~/.xhermes}/.env`
...
It does **not** turn XHermes into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to XHermes memory
...
### 1) "I want XHermes to own a real phone number"
Use **Twilio**.
...
- pairs well with XHermes `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${XHERMES_HOME:-~/.xhermes}/.env`
...
```bash
SCRIPT="$(find ~/.xhermes/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
xhermes skills search telephony
xhermes skills install official/productivity/telephony
```
...
Then save credentials into XHermes:
...
3. Buy it and save it into `${XHERMES_HOME:-~/.xhermes}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is XHermes calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing XHermes's existing `text_to_speech` support.

Use this when:
- you want the call to use XHermes's configured TTS voice rather than Twilio `<Say>`
...
Recommended XHermes TTS -> Twilio Play workflow:

1. Generate the audio with XHermes `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask XHermes to create the message audio with `text_to_speech`
...
Important note:
- XHermes TTS is great for prerecorded outbound messages
...
- XHermes STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to XHermes memory.
...
- Do not store arbitrary third-party phone numbers in XHermes memory.
...
3. persist that number to `${XHERMES_HOME:-~/.xhermes}/.env`
