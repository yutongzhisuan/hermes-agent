...
description: "Plugins shipped with XHermes Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

XHermes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.xhermes/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a XHermes Plugin](/developer-guide/plugins) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `xhermes memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `xhermes plugins list` and the interactive `xhermes plugins` UI), but none load until you explicitly enable them:

```bash
xhermes plugins enable disk-cleanup
```

Or via `~/.xhermes/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer XHermes. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
xhermes plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `xhermes plugins enable <name>`.
...
| `xhermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real XHermes session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `xhermes memory` and `xhermes plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Disabling again:** `xhermes plugins disable disk-cleanup`.
...
**Enabling:** `xhermes plugins enable security-guidance` (or check the box in `xhermes plugins`).

**Disabling again:** `xhermes plugins disable security-guidance`.
...
Traces XHermes turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of XHermes' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `xhermes logs`.
...
```bash
xhermes tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart XHermes and the next turn ships a trace.
...
xhermes chat -q "hello"              # check the Langfuse UI for a "XHermes turn" trace
```
...
**Disabling:** `xhermes plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, status) saved under `~/.xhermes/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
xhermes plugins enable google_meet
xhermes meet setup   # preflight: playwright, chromium, auth file
xhermes meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `xhermes plugins disable google_meet`. Any saved transcripts stay in `~/.xhermes/workspace/meetings/` until you remove them.

### xhermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real XHermes session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with XHermes feature changes.
...
| Secret | Hidden until XHermes detects the first related signal in your history |

**API** — routes mount under `/api/plugins/xhermes-achievements/`:
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across XHermes updates. |
...
Bundled plugins are written exactly like any other XHermes plugin — see [Build a XHermes Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.xhermes/plugins/<name>/`
- Manifest source is reported as `bundled` in `xhermes plugins list`
