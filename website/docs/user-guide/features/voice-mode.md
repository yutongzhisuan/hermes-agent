...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with XHermes Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

XHermes Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with XHermes](/guides/use-voice-mode-with-xhermes).

For hands-free session start — saying "hey xhermes" (or any phrase) to open a fresh voice session on the CLI, TUI, or desktop app — see [Wake Word](/user-guide/features/wake-word).

## Prerequisites

Before using voice features, make sure you have:

1. **XHermes Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `xhermes model` or set your preferred provider credentials in `~/.xhermes/.env`
3. **A working base setup** — run `xhermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.xhermes/` directory and default `config.yaml` are created automatically the first time you run `xhermes`. You only need to create `~/.xhermes/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `xhermes setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`xhermes chat`) and the **TUI** (`xhermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.xhermes/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
xhermes                # Start the interactive CLI
```
...
1. Start the CLI with `xhermes` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.xhermes/config.yaml` (default: `ctrl+b`).
:::
...
```bash
xhermes gateway        # Start the gateway (connects to configured platforms)
xhermes gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.xhermes/.env`:
```bash
...
```bash
xhermes gateway        # Start with existing configuration
```
...
If you are running XHermes inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.xhermes/.env`
...
- Review gateway logs: `tail -f ~/.xhermes/logs/gateway.log`
