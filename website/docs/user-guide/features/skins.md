...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the XHermes CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the XHermes CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.xhermes/skins/mytheme.yaml
```

Or set the default skin in `~/.xhermes/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic XHermes — gold and kawaii | `XHermes Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `XHermes Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `XHermes Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `XHermes Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `XHermes Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `XHermes Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to XHermes Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ XHermes ` |
...
Create YAML files under `~/.xhermes/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.xhermes/skins/mytheme.yaml
...
## XHermes Mod — Visual Skin Editor

[XHermes Mod](https://github.com/cocktailpeanut/xhermes-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![XHermes Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/xhermes-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all XHermes skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.xhermes/skins/`
- Activates a skin by updating `~/.xhermes/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/xhermes-mod.git
cd xhermes-mod/app
npm install
npm start
```
...
- User skins in `~/.xhermes/skins/` take precedence over built-in skins with the same name.
