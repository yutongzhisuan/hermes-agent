...
When XHermes Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.xhermes/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.xhermes/config.yaml` (switchable per session
via `/personality`).
...
If you use XHermes' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.xhermes/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. XHermes Agent's deliverable
