...
XHermes can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<XHERMES_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), XHermes renders the sprite at full fidelity when your
...
xhermes pets list
xhermes pets list cat

# Install a pet and make it active in one step
xhermes pets install boba --select
...
## `xhermes pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `xhermes pets list [query] [--limit N]` |
| List installed pets | `xhermes pets list --installed` |
| Install a pet | `xhermes pets install <slug> [--select] [--force]` |
...
| Preview/animate | `xhermes pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `xhermes pets off` |
| Remove an installed pet | `xhermes pets remove <slug>` |
| Diagnose setup | `xhermes pets doctor` |

`xhermes pets show` flags:
...
Beyond installing pre-made pets from the gallery, XHermes can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `xhermes pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `xhermes tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `XHERMES_PET_IMAGE_PROVIDER` env var (e.g. `XHERMES_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while XHermes is
...
Run `xhermes pets doctor` — it reports:
...
- The [`xhermes-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-xhermes-agent.md)
