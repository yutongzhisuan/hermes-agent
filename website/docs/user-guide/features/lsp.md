...
XHermes runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. XHermes
...
1. XHermes captures a baseline of current diagnostics for the file.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point XHermes at the extracted bundle — the directory that contains
...
`node_modules` tree — XHermes installs both packages together when you
run `xhermes lsp install typescript` or auto-install fires on first
use.

## CLI

```
xhermes lsp status          # service state + per-server install status
xhermes lsp list            # registry, optionally --installed-only
xhermes lsp install <id>    # eagerly install one server
xhermes lsp install-all     # try every server with a known recipe
xhermes lsp restart         # tear down running clients
xhermes lsp which <id>      # print resolved binary path
```

`xhermes lsp status` is the best starting point — it shows which
...
shared location — the staging dir is fully XHermes-owned and is
...
**`Backend warnings` section in `xhermes lsp status`**
...
When `xhermes lsp status` shows a `Backend warnings` section, install
...
`~/.xhermes/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.xhermes/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `xhermes lsp restart` to clear the set;
