...
Schedule tasks to run automatically with natural language or cron expressions. XHermes exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to XHermes itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `xhermes model`.

- **Per-job pin** — set by *you* via the dashboard, `xhermes cron create/edit --model … --provider …`, or by editing `~/.xhermes/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`xhermes config set cron.model <name>`) and switching your chat model with `xhermes model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `xhermes model`. In this case XHermes **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you to pin the provider/model explicitly (#44585). This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).

`xhermes setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. XHermes disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
xhermes cron create "every 2h" "Check server status"
xhermes cron create "every 1h" "Summarize new feed items" --skill blogwatcher
xhermes cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask XHermes normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

XHermes will use the unified `cronjob` tool internally.
...
```bash
xhermes config set cron.model_drift_guard false
```
...
xhermes cron create "every 1d at 09:00" \
...
```bash
xhermes cron edit <job_id> --schedule "every 4h"
xhermes cron edit <job_id> --prompt "Use the revised task"
xhermes cron edit <job_id> --skill blogwatcher --skill maps
xhermes cron edit <job_id> --add-skill maps
xhermes cron edit <job_id> --remove-skill blogwatcher
xhermes cron edit <job_id> --clear-skills
```
...
```bash
xhermes cron list
xhermes cron pause <job_id_or_name>
xhermes cron resume <job_id_or_name>
xhermes cron run <job_id_or_name>
xhermes cron remove <job_id_or_name>
xhermes cron edit <job_id_or_name> [...flags]
xhermes cron status
xhermes cron tick
```
...
sudo xhermes gateway install --system   # Linux: boot-time system service for servers
xhermes gateway             # Or run in foreground
...
On each tick XHermes:

1. loads jobs from `~/.xhermes/cron/jobs.json`
...
A file lock at `~/.xhermes/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

XHermes records each claimed cron attempt in the profile-local
`~/.xhermes/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, XHermes marks an abandoned
...
Inspect recent attempts with `xhermes cron runs [job-id] --limit 20` (alias:
...
| `"local"` | Save to local files only (`~/.xhermes/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.xhermes/cron/output/`), but no message is sent to the delivery target.
...
The `cronjob` tool's schema exposes `no_agent` to XHermes directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

XHermes will write the check script to `~/.xhermes/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.xhermes/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.xhermes/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.xhermes/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.xhermes/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, XHermes reads Job 1's most recent output from `~/.xhermes/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `xhermes tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `xhermes tools` cron-platform config wins; otherwise XHermes falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether XHermes should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.xhermes/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.xhermes/scripts/.feed-changed.last"
...
:::tip
XHermes's own `~/.xhermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/xhermes-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
