...
XHermes Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
xhermes memory setup      # interactive picker + configuration
xhermes memory status     # check what's active
xhermes memory off        # disable external provider
```

You can also select the active memory provider via `xhermes plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.xhermes/config.yaml`:
...
When a memory provider is active, XHermes automatically:
...
**Setup Wizard:**
```bash
xhermes memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `xhermes honcho setup` command still works (it now redirects to `xhermes memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$XHERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$XHERMES_HOME/honcho.json` > `~/.xhermes/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/xhermes-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/xhermes).
...
:::tip Migrating from `xhermes honcho`
If you previously used `xhermes honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per XHermes profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All XHermes profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per XHermes profile. Host key `xhermes` → default; `xhermes.<profile>` for others. |
...
`--clone` creates a `xhermes.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every XHermes profile, creates host blocks for any profile without one, inherits settings from the default `xhermes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `xhermes memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/xhermes-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/xhermes).
...
xhermes memory setup    # select "openviking"
# Or manually:
xhermes config set memory.provider openviking
```

`xhermes memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.xhermes/.env`, and for named profiles use
`~/.xhermes/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is XHermes' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
xhermes memory setup    # select "mem0" → "Platform"
# Or manually:
...
**Setup (OSS):**
```bash
xhermes memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
xhermes memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
xhermes memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
xhermes memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
xhermes memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.xhermes/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.xhermes/.env
```
...
**Config:** `$XHERMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.xhermes/.env`.
...
| `user_id` | `xhermes-user` | User identifier |
| `agent_id` | `xhermes` | Agent identifier |
...
**Switching modes:** Re-run `xhermes memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
xhermes memory setup    # select "hindsight"
# Or manually:
...
**Local mode UI:** `hindsight-embed -p xhermes ui start`

**Config:** `$XHERMES_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `xhermes` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between XHermes Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/xhermes-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
xhermes memory setup    # select "holographic"
# Or manually:
xhermes config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.xhermes-memory-store`
...
**Setup:**
```bash
xhermes memory setup    # select "retaindb"
# Or manually:
...
# Then configure XHermes
xhermes memory setup    # select "byterover"
# Or manually:
...
- Knowledge tree stored at `$XHERMES_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=xhermes), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
xhermes memory setup    # select "supermemory"
# Or manually:
...
Before running `xhermes memory setup`, set `base_url` in
`$XHERMES_HOME/supermemory.json`:
...
Then run `xhermes memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$XHERMES_HOME/supermemory.json`
...
| `container_tag` | `xhermes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `xhermes-{identity}` → `xhermes-coder`) to isolate memories per XHermes profile
...
| **Requires** | `pip install xhermes-memori` + `xhermes-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install xhermes-memori
xhermes-memori install
xhermes config set memory.provider memori
xhermes memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `xhermes-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$XHERMES_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$XHERMES_HOME/` so each profile has its own credentials
