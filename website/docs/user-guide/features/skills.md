...
All skills live in **`~/.xhermes/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point XHermes at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `xhermes update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.xhermes` profile):

```bash
curl -fsSL https://xhermes-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
xhermes profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
xhermes skills opt-out            # stop future seeding — nothing on disk is touched
xhermes skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
xhermes skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `xhermes update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `xhermes skills opt-in`) to re-enable.

:::note Safe by default
`xhermes skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version XHermes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling XHermes to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.xhermes/plans/` relative to the active workspace/backend working directory.
...
description, the standard section order, XHermes-tool framing, no invented
commands).
...
When a missing value is encountered, XHermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `xhermes setup` or `~/.xhermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `xhermes config migrate` prompts for unconfigured settings, and `xhermes config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
and `examples/`. Unreferenced repository files are not copied. XHermes scans the
...
If you maintain skills outside of XHermes — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell XHermes to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.xhermes/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.xhermes/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the XHermes process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, XHermes ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
Bundles live in **`~/.xhermes/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `xhermes bundles list`.
...
# Re-scan ~/.xhermes/skill-bundles/ and report changes
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.xhermes/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.xhermes/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.xhermes/pending/skills/` and are
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with XHermes. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `xhermes skills search <query> --source skills-sh`. XHermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
XHermes currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the XHermes repository itself and install with built-in trust.
...
This is Vercel's public skills directory. XHermes can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
XHermes can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
[Skills Hub](https://xhermes-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no XHermes-side changes required.
...
- XHermes source id: `clawhub`

#### 6. LobeHub (`lobehub`)

XHermes can search and convert agent entries from LobeHub's public catalog into installable XHermes skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- XHermes source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

XHermes integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with XHermes' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- XHermes source id: `browse-sh`
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). XHermes also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- XHermes source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. XHermes does not enumerate or copy unrelated files from the host.
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `xhermes skills update` re-fetches from the same URL automatically when you want to refresh.
...
`xhermes skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with XHermes | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other XHermes users add with `xhermes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.xhermes.tags`, `version`, `author`, `platforms`, `metadata.xhermes.config`).
...
XHermes discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any XHermes user can subscribe and install:
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.xhermes/skills/.hub/taps.json`:
...
The `xhermes skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `xhermes skills tap list` shows the effective path per tap.
...
Taps are stored in `~/.xhermes/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`xhermes skills reset`)

XHermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `xhermes update`, a sync pass copies those into `~/.xhermes/skills/` and records a manifest at `~/.xhermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, XHermes recomputes the hash of your local copy and compares it to the origin hash:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `XHERMES_HOME`, so `xhermes -p coder skills reset <name>` only affects that profile.
:::
