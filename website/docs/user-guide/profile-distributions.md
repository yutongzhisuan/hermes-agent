...
A **profile distribution** packages a complete XHermes agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a XHermes agent meant sending someone:
...
```bash
xhermes profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `xhermes profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running XHermes in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`xhermes profile export` / `import`](../reference/profile-commands.md#xhermes-profile-export) — that's what those are for.
...
:::caution
**XHermes does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `xhermes profile install` or `xhermes profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
xhermes profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.xhermes/profiles/research-bot/SOUL.md
...
Create `~/.xhermes/profiles/research-bot/distribution.yaml`:
...
Create `~/.xhermes/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.xhermes/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `xhermes profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.xhermes/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
xhermes profile install github.com/you/research-bot

# Full HTTPS
xhermes profile install https://github.com/you/research-bot.git

# SSH
xhermes profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
xhermes profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
xhermes profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
xhermes profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
xhermes profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
xhermes profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.xhermes/profiles/research-bot/.env.EXAMPLE ~/.xhermes/profiles/research-bot/.env
...
```bash
xhermes profile info research-bot
```
...
`xhermes profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
xhermes profile delete research-bot
```
...
Path:    ~/.xhermes/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `xhermes profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.xhermes/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
xhermes profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `xhermes profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.xhermes/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/xhermes-polymarket-trader.git
git push -u origin main --tags

# Anyone
xhermes profile install github.com/you/xhermes-polymarket-trader --alias
```
...
You built XHermes-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
xhermes profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
xhermes profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `xhermes profile info <name>` and hold off on updates until you're ready.
:::
...
xhermes profile info research-bot | grep Version
...
```yaml
# ~/.xhermes/profiles/research-bot/local/my-overrides.yaml
...
xhermes profile delete research-bot --yes
xhermes profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
xhermes profile update research-bot --force-config --yes
```
...
xhermes profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.xhermes/profiles/forked-research-bot/
...
xhermes profile install ~/.xhermes/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
xhermes profile delete research-bot-test --yes
xhermes profile install ~/.xhermes/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `xhermes -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `xhermes profile` — not a parallel command tree.
...
- Reserved profile names (`xhermes`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`xhermes profile export` / `import`](../reference/profile-commands.md#xhermes-profile-export) — local backup / restore (not distribution)
- [Using SOUL with XHermes](../guides/use-soul-with-xhermes.md) — authoring personalities
