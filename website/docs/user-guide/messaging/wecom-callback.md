...
Connect XHermes to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
XHermes supports two WeCom integration modes:
...
> Run `xhermes gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. XHermes decrypts the message, queues it for the agent
...
(Use `xhermes gateway start` only after `xhermes gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in XHermes and the token the
...
in `~/.xhermes/.env` values around `=` will also break signature checks. After
fixing, restart `xhermes gateway run`.
...
Check `xhermes gateway run` logs for the bound host/port. If the adapter bound to
