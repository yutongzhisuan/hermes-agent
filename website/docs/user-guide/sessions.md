...
XHermes Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.xhermes/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
XHermes stores session history so it can resume conversations, but it does not
...
XHermes explicitly injects for that turn.
...
For example, if a user sends an image and asks XHermes to make a meme from it,
XHermes may inspect that image once with vision and run an image-processing
...
`xhermes sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`xhermes` or `xhermes chat`) |
...
xhermes -c "my project"   # → resumes "my project #3"
```
...
Session IDs are shown when you exit a CLI session, and can be found with `xhermes sessions list`.
...
```bash
xhermes --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`xhermes sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
xhermes sessions list --workspace my-project
xhermes sessions list --workspace ~/code/xhermes-agent
```

### Conversation Recap on Resume

When you resume a session, XHermes displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a XHermes session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.xhermes/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `xhermes -r "<title>"` from the shell) and pick up where the platform left off.
...
XHermes automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `xhermes sessions list` or `xhermes sessions browse`.
...
When a session's context is compressed (manually via `/compress` or automatically), XHermes creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`xhermes -c "my project"`), it automatically picks the most recent session in the lineage.
...
XHermes provides a full set of session management commands via `xhermes sessions`:
...
`xhermes sessions export` is one surface for every export format, selected with `--format`:
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `xhermes-traces` dataset (reads `HF_TOKEN`):
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.xhermes/session-exports`).
...
xhermes sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
xhermes sessions prune --newer-than 5h

# Explicit window with absolute timestamps
xhermes sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
xhermes sessions prune --source telegram
xhermes sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
xhermes sessions prune --newer-than 5h --title "smoke test"   # title substring
xhermes sessions prune --older-than 30 --max-messages 3        # tiny sessions
xhermes sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
xhermes sessions prune --model gpt-5 --older-than 1w           # by model (substring)
xhermes sessions prune --provider openrouter --older-than 60   # by billing provider
xhermes sessions prune --branch feature/old-experiment         # by git branch
xhermes sessions prune --user 12345678 --chat-type group       # by messaging origin
xhermes sessions prune --max-tokens 500 --older-than 7         # by token usage
xhermes sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
xhermes sessions prune --newer-than 5h --dry-run
...
the implicit 90-day default, so `xhermes sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `xhermes sessions prune` keeps the 90-day cutoff. Every
...
`xhermes sessions archive` takes the same filters as `prune` but soft-hides
...
xhermes sessions archive --newer-than 5h

# Archive by title substring, preview first
xhermes sessions archive --title "dry run" --dry-run
xhermes sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `xhermes sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`xhermes sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`xhermes insights`](/reference/cli-commands#xhermes-insights).
...
When XHermes cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, XHermes uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to XHermes in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.xhermes/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.xhermes/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.xhermes/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.xhermes/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.xhermes/sessions/sessions.json` is a **legacy mirror** of it,
...
`xhermes sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.xhermes/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `xhermes sessions list`, the cause is
`state.db` not receiving them — run `xhermes sessions repair` and watch for a
...
`*.jsonl` files in `~/.xhermes/sessions/`. They are no longer written or
read by XHermes. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every XHermes process in the same `XHERMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.xhermes/config.yaml`:
