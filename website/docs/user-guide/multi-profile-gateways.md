...
If you only run one XHermes agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more XHermes agents that should all
...
(`ai.xhermes.gateway-<name>.plist`) or systemd user service
(`xhermes-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
xhermes profile create coder
xhermes profile create personal-bot
xhermes profile create research
...
Equivalently, in the default profile's `~/.xhermes/config.yaml`:
...
You do **not** run `xhermes gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `xhermes gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.xhermes/profiles/coder/.env`; the default listener key is rejected.
...
home). `xhermes status` reports the multiplexer and the profiles it serves;
`xhermes status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/xhermes-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: xhermes-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    xhermes gateway "$action"
  else
    xhermes -p "$profile" gateway "$action"
  fi
}
...
```bash
xhermes-gateways start      # start every configured profile
xhermes-gateways stop       # stop every configured profile
xhermes-gateways restart    # restart all
xhermes-gateways status     # status across all
xhermes-gateways list       # delegates to `xhermes gateway list`
```

:::tip
The `default` profile is targeted with `xhermes gateway <action>` (no `-p`),
not `xhermes -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `xhermes -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.xhermes.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/xhermes-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.xhermes.gateway.plist` /
`xhermes-gateway.service`.
...
tail -f ~/.xhermes/logs/gateway.error.log

# Named profile
tail -f ~/.xhermes/profiles/<name>/logs/gateway.log
tail -f ~/.xhermes/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.xhermes/logs/gateway.log ~/.xhermes/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
xhermes logs -f                  # follow default profile
xhermes -p coder logs -f         # follow one profile
xhermes logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
xhermes profile list             # profiles + model + gateway state
xhermes-gateways status          # full status across every profile
launchctl list | grep xhermes    # macOS — PIDs and labels
systemctl --user list-units 'xhermes-gateway-*'   # Linux — units
```
...
```
~/.xhermes/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.xhermes/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
xhermes config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.xhermes/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=xhermes --why="gateways running" \
  sleep infinity &
...
`xhermes-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.xhermes/.env ~/.xhermes/profiles/*/.env
```

## Updating the code

`xhermes update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `xhermes gateway start` after a previous `xhermes gateway stop`. The
...
cat ~/.xhermes/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.xhermes.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.xhermes.gateway-<profile>.plist

# Linux
systemctl --user restart xhermes-gateway-<profile>.service
```

### Health check

```bash
xhermes doctor                  # default profile
xhermes -p <profile> doctor     # one profile
```
