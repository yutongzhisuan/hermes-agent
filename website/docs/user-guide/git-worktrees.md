...
title: "Git Worktrees"
description: "Run multiple XHermes agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

XHermes Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with XHermes so each session has a clean, isolated working directory.

## Why Use Worktrees with XHermes?

XHermes treats the **current working directory** as the project root:

- CLI: the directory where you run `xhermes` or `xhermes chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.xhermes/config.yaml`
...
git worktree add ../repo-feature feature/xhermes-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/xhermes-experiment` checked out in that directory

Now you can `cd` into the new worktree and run XHermes there:

```bash
cd ../repo-feature

# Start XHermes in the worktree
xhermes
```
...
git worktree add ../repo-experiment-a feature/xhermes-a
git worktree add ../repo-experiment-b feature/xhermes-b
```
...
Each XHermes process:

- Works on its own branch (`feature/xhermes-a` vs `feature/xhermes-b`).
...
- XHermes checkpoint data under `~/.xhermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per XHermes experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/xhermes-checkpoints-docs`, `feature/xhermes-refactor-tests`.
- **Commit frequently**
...
- **Avoid running XHermes from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `xhermes -w` (Automatic Worktree Mode)

XHermes has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `xhermes/xhermes-<hash>`).
...
```bash
xhermes -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `xhermes -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each XHermes session its own clean checkout.
