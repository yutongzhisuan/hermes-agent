---
sidebar_position: 7
title: "Use SOUL.md with XHermes"
description: "How to use SOUL.md to shape XHermes Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with XHermes

`SOUL.md` is the **primary identity** for your XHermes instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want XHermes to feel like the same assistant every time you talk to it — or if you want to replace the XHermes persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm XHermes should be
- what XHermes should avoid stylistically
- how XHermes should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who XHermes is and how XHermes speaks
...
XHermes now uses only the global SOUL file for the current instance:

```text
~/.xhermes/SOUL.md
```

If you run XHermes with a custom home directory, it becomes:
...
XHermes automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, XHermes does not overwrite it
- if the file exists but is empty, XHermes adds nothing from it to the prompt

## How XHermes uses it

When XHermes starts a session, it reads `SOUL.md` from `XHERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how XHermes feels.
...
XHermes already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How XHermes should sound.

# Avoid
What XHermes should not do.

# Defaults
How XHermes should behave when ambiguity appears.
```
...
Then restart XHermes or start a new session.
...
### I edited SOUL.md but XHermes still sounds the same

Check:
- you edited `~/.xhermes/SOUL.md` or `$XHERMES_HOME/SOUL.md`
...
### XHermes is ignoring parts of my SOUL.md
