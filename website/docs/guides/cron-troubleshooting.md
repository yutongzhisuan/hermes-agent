...
description: "Diagnose and fix common XHermes cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`xhermes gateway` for foreground, or `xhermes gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `xhermes cron tick`.
...
```bash
date
xhermes cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.xhermes/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.xhermes/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.xhermes/.env` |
...
| `local` | Write access to `~/.xhermes/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `xhermes cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `xhermes skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `xhermes skills list`.
...
2. `~/.xhermes/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `xhermes cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the XHermes config directory). Verify:
```bash
ls ~/.xhermes/scripts/your-script.py   # Must exist
xhermes cron edit <job_id> --script ~/.xhermes/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `xhermes skills install <skill-name>`.
...
Jobs are stored in `~/.xhermes/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.xhermes/cron/jobs.json
chmod 600 ~/.xhermes/cron/jobs.json   # Your user should own it
```
...
```bash
xhermes cron list                    # Show all jobs, states, next_run times
xhermes cron run <job_id>            # Schedule for next tick (for testing)
xhermes cron edit <job_id>           # Fix configuration issues
xhermes logs                         # View recent XHermes logs
xhermes skills list                  # Verify installed skills
```
...
1. Run the job with `xhermes cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.xhermes/logs/agent.log` for scheduler messages and `~/.xhermes/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/xhermes-agent](https://github.com/NousResearch/xhermes-agent) with:
