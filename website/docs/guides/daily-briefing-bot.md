...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `xhermes setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **XHermes spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.xhermes/cron/output/` and you can read them anytime.
:::
...
XHermes will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by XHermes Agent
```
...
Before creating cron jobs, ensure XHermes has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell XHermes what you want:
...
XHermes will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell XHermes to delegate each topic to a sub-agent:
...
XHermes will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
