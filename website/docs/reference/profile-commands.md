...
This page covers all commands related to [XHermes profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `xhermes profile` without a subcommand shows help.
...
```bash
xhermes profile use <name>
```

Sets `<name>` as the active profile. All subsequent `xhermes` commands (without `-p`) will use this profile.
...
xhermes profile use default
```

## `xhermes profile create`

```bash
xhermes profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `xhermes profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `xhermes update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.xhermes`), use `xhermes skills opt-out` / `xhermes skills opt-in`. |
...
xhermes profile create work2 --clone-from work

# Clone everything from a specific profile
xhermes profile create work2-backup --clone-from work --clone-all
```

## `xhermes profile describe`

```bash
xhermes profile describe [<name>] [options]
```
...
xhermes profile describe researcher

# Set it explicitly
xhermes profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
xhermes profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
xhermes profile describe --all --auto
```

## `xhermes profile delete`

```bash
xhermes profile delete <name> [options]
```
...
```bash
xhermes profile delete mybot
xhermes profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.xhermes`) cannot be deleted — use `xhermes uninstall` to remove everything.
:::

## `xhermes profile show`

```bash
xhermes profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's XHermes home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ xhermes profile show work
Profile: work
Path:    ~/.xhermes/profiles/work
...
## `xhermes profile alias`

```bash
xhermes profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your XHermes installation.
...
```bash
xhermes profile alias work
# Creates/updates ~/.local/bin/work

xhermes profile alias work --name mywork
# Creates ~/.local/bin/mywork

xhermes profile alias work --remove
# Removes the wrapper script
```

## `xhermes profile rename`

```bash
xhermes profile rename <old-name> <new-name>
```
...
```bash
xhermes profile rename mybot assistant
# ~/.xhermes/profiles/mybot → ~/.xhermes/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `xhermes profile export`

```bash
xhermes profile export <name> [options]
```
...
```bash
xhermes profile export work
# Creates work.tar.gz in the current directory

xhermes profile export work -o ./work-2026-03-29.tar.gz
```

## `xhermes profile import`

```bash
xhermes profile import <archive> [options]
```
...
```bash
xhermes profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

xhermes profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`xhermes profile export` / `import` are still the right commands for
...
### `xhermes profile install`

```bash
xhermes profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `xhermes -p telemetry`). |
...
xhermes profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
xhermes profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
xhermes profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
xhermes profile install ./telemetry/
```

### `xhermes profile update`
...
XHermes version, author, env var requirements, the source URL/path, and
...
`xhermes profile list` also shows the distribution name and version in a
`Distribution` column, and `xhermes profile show <name>` / `delete <name>`
...
xhermes profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
xhermes profile install https://github.com/your-org/internal-assistant.git
```
...
XHermes version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host XHermes can clone from.
3. Tell recipients to run `xhermes profile install <your-repo-url>`.
...
## `xhermes -p` / `xhermes --profile`

```bash
xhermes -p <name> <command> [options]
xhermes --profile <name> <command> [options]
```

Global flag to run any XHermes command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
xhermes -p work chat -q "Check the server status"
xhermes --profile dev gateway start
xhermes -p personal skills list
xhermes -p work config edit
```

## `xhermes completion`

```bash
xhermes completion <shell>
```
...
- `xhermes profile <TAB>` — subcommands (list, use, create, etc.)
- `xhermes profile use <TAB>` — profile names
- `xhermes -p <TAB>` — profile names
