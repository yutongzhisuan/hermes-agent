...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with XHermes Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working XHermes setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [XHermes Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up XHermes for a team, bot, or always-on workflow
...
|---|---|---|
| I just want XHermes working on my machine | `xhermes setup` | Run a real chat and verify it responds |
| I already know my provider | `xhermes model` | Save the config, then start chatting |
| I want a bot or always-on setup | `xhermes gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `xhermes model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `xhermes model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if XHermes cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `xhermes model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `xhermes setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `xhermes update`. Re-enable anything later with `xhermes tools`, seed skills with `xhermes skills opt-in --sync`, or tune settings with `xhermes setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `xhermes model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `xhermes model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `xhermes model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `xhermes model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `xhermes model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `xhermes model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `xhermes model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `xhermes model` (requires `copilot` CLI + `copilot login`) |
...
XHermes Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `xhermes model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

XHermes separates secrets from normal config:

- **Secrets and tokens** → `~/.xhermes/.env`
- **Non-secret settings** → `~/.xhermes/config.yaml`
...
```bash
xhermes            # classic CLI
xhermes --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
XHermes ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `xhermes` vs `xhermes --tui`.
:::
...
```bash
xhermes --continue    # Resume the most recent session
xhermes -c            # Short form
```
...
```bash
xhermes gateway setup    # Interactive platform configuration
```
...
- `xhermes tools` — tune tool access per platform
- `xhermes skills` — browse and install reusable workflows
...
```bash
xhermes config set terminal.backend docker    # Docker isolation
xhermes config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `xhermes egress setup && xhermes egress start`; `xhermes setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the XHermes install directory (the curl installer placed it at
# ~/.xhermes/xhermes-agent on Linux/macOS or %LOCALAPPDATA%\xhermes\xhermes-agent on Windows):
...
Skills are on-demand instruction documents that teach XHermes how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

XHermes ships with a catalog of bundled skills already installed in `~/.xhermes/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
xhermes skills browse                      # list everything available
xhermes skills search kubernetes           # find skills by keyword
xhermes skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `xhermes skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let XHermes ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.xhermes/xhermes-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| XHermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `xhermes model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `xhermes gateway setup` and check `xhermes gateway status` |
| `xhermes --continue` can't find old session | Switched profiles or session never saved | Check `xhermes sessions list` and confirm you're in the right profile |
...
| `xhermes doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `xhermes tools` | Configure which tools are enabled per platform |
| `xhermes setup` | Full setup wizard (configures everything at once) |
| `xhermes doctor` | Diagnose issues |
| `xhermes update` | Update to latest version |
| `xhermes gateway` | Start the messaging gateway |
| `xhermes --continue` | Resume last session |
