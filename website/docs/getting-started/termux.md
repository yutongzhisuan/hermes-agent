...
description: "Run XHermes Agent directly on an Android phone with Termux"
---

# XHermes on Android with Termux
...
XHermes Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop XHermes from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

XHermes now ships a Termux-aware installer path:
...
- links `xhermes` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `xhermes` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/xhermes" "$PREFIX/bin/xhermes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `xhermes` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.xhermes/.env`.
...
### `xhermes doctor` says ripgrep or Node is missing
