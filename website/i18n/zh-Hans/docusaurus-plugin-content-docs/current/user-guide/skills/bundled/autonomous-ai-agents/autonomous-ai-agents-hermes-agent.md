...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_XHERMES_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export XHERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `xhermes config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `xhermes kanban <verb>` 驱动；调度器生成的 worker 看到由 `XHERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
- **隔离：** 看板是硬边界（worker 在环境中固定 `XHERMES_KANBAN_BOARD`）；租户是看板内用于工作区路径和记忆键隔离的软命名空间。
...
**2. 添加到 `toolsets.py`** → `_XHERMES_CORE_TOOLS` 列表。
...
- 测试自动将 `XHERMES_HOME` 重定向到临时目录——永远不会触及真实的 `~/.xhermes/`
- 推送任何变更前运行完整套件
