...
如果运维人员确实需要修复或检查 `/opt/data` 之外的文件，请有意识地使用 root shell。`xhermes` shim 默认会把 `docker exec xhermes xhermes ...` 降回运行时用户；只有在你明确需要 root 语义时，才临时设置 `HERMES_DOCKER_EXEC_AS_ROOT=1`。
...
容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `xhermes`（UID 10000）。如果宿主机的 `~/.xhermes/` 由不同 UID 拥有，请设置 `HERMES_UID`/`HERMES_GID` 以匹配宿主机用户，或确保数据目录可写：
