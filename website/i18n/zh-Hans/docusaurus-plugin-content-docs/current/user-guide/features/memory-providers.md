...
**配置：** `$XHERMES_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$XHERMES_HOME/honcho.json` > `~/.xhermes/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/xhermes-ai/xhermes-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/xhermes)。
...
**配置：** `$XHERMES_HOME/hindsight/config.json`
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$XHERMES_HOME/memory_store.db` | SQLite 数据库路径 |
...
- 知识树存储于 `$XHERMES_HOME/byterover/`（profile 范围隔离）
...
`$XHERMES_HOME/supermemory.json` 中设置 `base_url`：
...
**配置：** `$XHERMES_HOME/supermemory.json`
...
- **本地存储提供者**（Holographic、ByteRover）使用 `$XHERMES_HOME/` 路径，各 profile 路径不同
- **配置文件提供者**（Honcho、Mem0、Hindsight、Supermemory）将配置存储在 `$XHERMES_HOME/` 中，每个 profile 拥有独立凭证
