...
| `XHERMES_COPILOT_ACP_COMMAND` | 覆盖 Copilot CLI 二进制路径（默认：`copilot`） |
| `XHERMES_COPILOT_ACP_ARGS` | 覆盖 ACP 参数（默认：`--acp --stdio`） |
...
仅在 portal 端点迁移时才需设置 `XHERMES_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
