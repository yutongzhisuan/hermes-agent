...
React + Ink terminal UI for XHermes. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `XHERMES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `xhermes setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.xhermes/.hermes_history` or under `XHERMES_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `xhermes setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/xhermes-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external xhermes setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.xhermes/tui-parent.log
    ...
    types/
      xhermes-ink.d.ts            type declarations for @xhermes/ink
