...
// Profile-aware: the Python launcher exports XHERMES_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.xhermes matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.XHERMES_HOME ?? join(homedir(), '.xhermes'), 'tui-theme-boot.json')

// Never touch the user's real ~/.xhermes from test runs (the TS suite has no
// XHERMES_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into XHERMES_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.XHERMES_TUI_THEME || env.XHERMES_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.XHERMES_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.XHERMES_TUI_BACKGROUND
  ) {
    env.XHERMES_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.XHERMES_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.XHERMES_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into XHERMES_TUI_THEME.
