...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.XHERMES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep XHermes on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via XHERMES_TUI_TRUECOLOR=1.
