...
//
// Both gate on XHERMES_DEV_PERF=1 and dump JSON-lines (default ~/.xhermes/perf.log,
// override XHERMES_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// XHERMES_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.XHERMES_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.XHERMES_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.XHERMES_DEV_PERF_LOG?.trim() || join(homedir(), '.xhermes', 'perf.log')
