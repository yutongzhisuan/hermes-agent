...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $XHERMES_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see XHERMES_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['XHERMES_TUI_BACKGROUND', process.env.XHERMES_TUI_BACKGROUND ?? '(unset)'],
            ['XHERMES_TUI_THEME', process.env.XHERMES_TUI_THEME ?? '(unset)'],
