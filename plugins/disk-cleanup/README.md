...
Auto-tracks and cleans up ephemeral files created during XHermes Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$XHERMES_HOME` and `/tmp/xhermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `XHERMES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `XHERMES_HOME` or `/tmp/xhermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$XHERMES_HOME/disk-cleanup/` is itself excluded
- `$XHERMES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
