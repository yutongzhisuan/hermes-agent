"""disk_cleanup — ephemeral file cleanup for xHermes Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under XHERMES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly XHERMES_HOME and /tmp/xhermes-*
Never touches: ~/.xhermes/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("XHERMES_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".xhermes").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$XHERMES_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$XHERMES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under XHERMES_HOME or ``/tmp/xhermes-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/xhermes-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("xhermes-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "xhermes-agent", "backups", "profiles", ".worktrees",
...
# Paths under $XHERMES_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so XHERMES_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside XHERMES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under XHERMES_HOME, but never recurse into known
    # durable state trees.  Some installs place the XHermes checkout, venv,
    # and desktop build under XHERMES_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under XHERMES_HOME (e.g. /tmp/xhermes-*) — fall through.
        pass
