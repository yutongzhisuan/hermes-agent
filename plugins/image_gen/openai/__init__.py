...
three virtual model IDs so the ``xhermes tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `xhermes tools` → Image "
                    "Generation → OpenAI to configure, or `xhermes setup` "
                    "to add the key."
                ),
