"""`xhermes meet node ...` subcommand tree.

Wired into the existing ``xhermes meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``xhermes meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="xhermes-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``xhermes meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             xhermes meet node approve <name> ws://<host>:{args.port} {token}")
        try:
