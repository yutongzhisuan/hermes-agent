...
Let the xhermes agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where xhermes runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `xhermes meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `xhermes meet` CLI |
...
| `cli.py` | `xhermes meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$XHERMES_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `xhermes meet node {run,list,approve,remove,status,ping}` |
...
xhermes meet install                                      # pip + Chromium
xhermes meet setup                                        # preflight
xhermes meet auth                                         # optional
xhermes meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
xhermes meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.xhermes/.env
xhermes meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
xhermes meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
xhermes meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.xhermes/.env
xhermes meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, xhermes will **not** switch your system audio input automatically — the
...
xhermes plugins enable google_meet
xhermes meet node run --display-name my-mac --host 0.0.0.0 --port 18789
