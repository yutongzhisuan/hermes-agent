...
Configuration
-------------
Secret (lives in $XHERMES_HOME/.env or the environment):
...
Behavioral settings (live in $XHERMES_HOME/mem0.json, set via `xhermes memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $XHERMES_HOME/mem0.json overrides.
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $XHERMES_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $XHERMES_HOME/mem0.json) —
