...
```bash
xhermes memory setup    # select "mem0"
```
...
Behavioral settings live in `$XHERMES_HOME/mem0.json` (set them via `xhermes memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.xhermes/.env`.
...
| `user_id` | `xhermes-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh XHermes session and call `mem0_search` — it connects to your server.
...
```bash
xhermes memory setup mem0 --mode oss \
...
```bash
xhermes memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$XHERMES_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.xhermes/mem0_qdrant"}}
  }
}
```
...
```bash
xhermes memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$XHERMES_HOME/mem0.json`).
