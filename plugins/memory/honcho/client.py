...
Resolution order for config file:
  1. $XHERMES_HOME/honcho.json  (instance-local, enables isolated XHermes instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a XHermes profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active XHermes profile.

    Resolution order:
      1. XHERMES_HONCHO_HOST env var (explicit override)
      ...
      4. Fallback: ``"xhermes"`` (default profile)
    """
    explicit = os.environ.get("XHERMES_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named XHermes
    # profile share that same host.  Keep named profiles isolated; only the
    # default XHermes profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $XHERMES_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.xhermes/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.xhermes
    ...
    def from_global_config(
        ...
        Resolution: $XHERMES_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active XHermes profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.xhermes block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — XHermes session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. XHermes session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'xhermes honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `xhermes honcho setup` to configure)."
            )
