...
- Hosted: API key from [app.supermemory.ai/integrations?connect=xhermes](http://app.supermemory.ai/integrations?connect=xhermes)
...
Before running `xhermes memory setup`, add the local endpoint to
...
Then run `xhermes memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$XHERMES_HOME/supermemory.json`
...
| `container_tag` | `xhermes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `xhermes-{identity}` → `xhermes-coder`). |
...
`https://api.supermemory.ai`. XHermes resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: xhermes`, and document writes stamp
`metadata.sm_source: xhermes`. This is a **functional routing key, not telemetry**:
it groups XHermes-written memories into a dedicated "XHermes" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per XHermes profile:

```json
{
  "container_tag": "xhermes-{identity}"
}
```

For a profile named `coder`, this resolves to `xhermes-coder`. The default profile resolves to `xhermes-default`. Without `{identity}`, all profiles share the same container.
