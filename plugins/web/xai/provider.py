...
``{title, url, description, position}`` rows every other XHermes web
provider produces.
...
prefers XHermes-managed xAI Grok OAuth (via ``xhermes auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.xhermes/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``xhermes tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `xhermes auth` to sign in with "
