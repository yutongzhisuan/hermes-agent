# XHermes Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: xhermes-achievements plugin
...
### `GET /api/plugins/xhermes-achievements/achievements`
...
### `POST /api/plugins/xhermes-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/xhermes-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/xhermes-achievements/overview`
...
- `~/.xhermes/plugins/xhermes-achievements/`
