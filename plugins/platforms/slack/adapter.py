...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `xhermes slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `xhermes gateway setup` "
                "or in the active profile's ~/.xhermes/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `xhermes gateway setup` "
                "or add it to your active profile's ~/.xhermes/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `xhermes gateway setup` "
                "or in the active profile's ~/.xhermes/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `xhermes gateway setup` "
                "or add it to your active profile's ~/.xhermes/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            # XHermes has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by XHermes can "
                    "be removed from the Slack app manifest via "
                    "`xhermes slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /xhermes prefix). A single regex
            ...
            # manifest (see `xhermes slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/xhermes$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'xhermes slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: XHermes handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own XHermes session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # XHermes owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer XHermes agents in Socket Mode on some
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/xhermes <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/xhermes
        ...
        if slash_name in {"xhermes", ""}:
            # Legacy /xhermes <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/xhermes   ``).
        ...
        # questions via "/xhermes <question>" must produce public replies so
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="xHermes",
                bot_description="Your XHermes agent on Slack",
            )
            ...
            print_info(
                "   Re-run `xhermes slack manifest --write` anytime to refresh after "
                "XHermes adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `xhermes update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://xhermes-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `xhermes setup` to install Slack support.",
