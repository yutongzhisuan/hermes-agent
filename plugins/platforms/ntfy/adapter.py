"""ntfy platform adapter (XHermes plugin).
...
a XHermes dependency.

This adapter ships as a XHermes platform plugin under
``plugins/platforms/ntfy/``. The XHermes plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "xhermes-in"              # subscribe topic (incoming)
          publish_topic: "xhermes-out"     # optional — defaults to topic
...
_ECHO_TAG = "xhermes-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``xhermes cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a XHermes dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `xhermes gateway status` without
