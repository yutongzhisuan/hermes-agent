"""
IRC Platform Adapter for xHermes Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the XHermes agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "xhermes-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :xHermes Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :xHermes Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # xhermes setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `xhermes gateway setup` flow for the IRC platform.
    ...
    print_info("Connect XHermes to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #xhermes — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.xhermes/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "xhermes-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "xhermes-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :xHermes Agent (cron)")
