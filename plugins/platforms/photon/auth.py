...
Credential storage mirrors every other XHermes channel:

    * runtime SDK creds  -> ``~/.xhermes/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.xhermes/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a xhermes dependency
...
# registers XHermes as its own client_id.
...
# Default name of the project XHermes provisions for the operator.
DEFAULT_PROJECT_NAME = "xHermes Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of xhermes-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.xhermes/auth.json`` honouring the active XHermes profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.xhermes")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.xhermes/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.xhermes/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.xhermes/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `xhermes photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `xhermes photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `xhermes photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `xhermes photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `xhermes photon setup`)"
