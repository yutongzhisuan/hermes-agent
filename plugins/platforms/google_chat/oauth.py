...
- Per-user tokens (keyed by sender email):
    ``${XHERMES_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${XHERMES_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${XHERMES_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${XHERMES_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${XHERMES_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's XHERMES_HOME helper so the token follows the user's
# profile (e.g. tests can override via XHERMES_HOME=/tmp/...).
try:
...
def _hermes_home() -> Path:
    """Resolve XHERMES_HOME at call time (NOT module import).

    Tests and ``XHERMES_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.xhermes/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `xhermes setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `xhermes setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into XHERMES_HOME."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for XHermes (native attachment delivery)"
    )
