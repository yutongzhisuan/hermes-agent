...
# ``model_tools`` import time, so ``xhermes status``, ``xhermes chat``, etc.
...
#     fresh "XHermes is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a xhermes MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "xhermes-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${XHERMES_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "XHermes is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("XHERMES_HOME", str(get_hermes_home()))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${XHERMES_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a xhermes MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'XHermes is thinking…' marker message.
        ...
        "XHermes is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "XHermes is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "XHermes is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "XHermes is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``xhermes setup``.
    ...
    prompt for env vars, persist them to ``~/.xhermes/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. xhermes-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.xhermes/.env")
    print_info("Restart the gateway: xhermes gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``xhermes cron`` running as a
    separate process from ``xhermes gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `xhermes setup` to install Google Chat support.",
        ...
        # the "XHermes is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'XHermes is thinking…' marker message "
