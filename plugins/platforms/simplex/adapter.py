"""SimpleX Chat platform adapter (XHermes plugin).
...
This adapter ships as a XHermes platform plugin under
``plugins/platforms/simplex/``. The XHermes plugin loader scans the
...
Optional environment variables:
    ...
    XHERMES_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``xhermes setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside XHermes — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("XHERMES_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``xhermes setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.xhermes/.env`` via
    ``hermes_cli.config``.
    """
    print()
