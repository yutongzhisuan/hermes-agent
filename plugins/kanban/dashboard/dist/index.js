/**
 * XHermes Kanban — Dashboard Plugin
 *
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "xhermes-kanban-dot-triage",
    todo: "xhermes-kanban-dot-todo",
    ready: "xhermes-kanban-dot-ready",
    running: "xhermes-kanban-dot-running",
    blocked: "xhermes-kanban-dot-blocked",
    done: "xhermes-kanban-dot-done",
    archived: "xhermes-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-xhermes-task";
  ...
  const DOCS_URL = "https://xhermes-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://xhermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "xhermes.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="xhermes-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])xhermes-kanban-md-code\1/i.test(attrs)) {
      return ' class="xhermes-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "xhermes-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "xhermes-kanban-md",
  ...
  // standard `drop` event and our `xhermes-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("xhermes-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("xhermes-kanban-column--drop");
          if (target) target.classList.add("xhermes-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("xhermes-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("xhermes-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("xhermes-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "xhermes-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "xhermes-kanban-attention",
        "xhermes-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "xhermes-kanban-attention-bar" },
        h("span", { className: "xhermes-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "xhermes-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "xhermes-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "xhermes-kanban-attention-row",
                  "xhermes-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "xhermes-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "xhermes-kanban-attention-row-id" }, task.id),
                h("span", { className: "xhermes-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "xhermes-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "xhermes-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "xhermes-kanban-diag-action-btn",
      action.suggested ? "xhermes-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " xhermes-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".xhermes-kanban-drawer-comment-row input, .xhermes-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "xhermes-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("xhermes-kanban-diag", sevClass) },
      h("div", { className: "xhermes-kanban-diag-header" },
        h("span", { className: "xhermes-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "xhermes-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "xhermes-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "xhermes-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "xhermes-kanban-diag-data-row" },
                  h("span", { className: "xhermes-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "xhermes-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "xhermes-kanban-diag-data-row" },
                h("span", { className: "xhermes-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "xhermes-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "xhermes-kanban-diag-reassign-row" },
            h("span", { className: "xhermes-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "xhermes-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "xhermes-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "xhermes-kanban-diag-msg",
              msg.ok ? "xhermes-kanban-diag-msg--ok" : "xhermes-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "xhermes-kanban-section" },
      h("div", { className: "xhermes-kanban-section-head-row" },
        h("span", { className: "xhermes-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "xhermes-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "xhermes-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "xhermes-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "xhermes-kanban-docs-link",
      title: "Open XHermes Kanban docs in a new tab",
      "aria-label": "XHermes Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `xhermes kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "xhermes-kanban-msg-ok" : "xhermes-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "xhermes-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "xhermes-kanban-boardswitcher" },
      h("div", { className: "xhermes-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "xhermes-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "xhermes-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "xhermes-kanban-dialog-title" },
        ...
        h("div", { className: "xhermes-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned XHermes profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "xhermes-kanban-bulk" },
      h("span", { className: "xhermes-kanban-bulk-count" },
      ...
      h("div", { className: "xhermes-kanban-bulk-priority",
      ...
      h("div", { className: "xhermes-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different XHermes profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "xhermes-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("xhermes-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("xhermes-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "xhermes-kanban-trash",
        dragOver ? "xhermes-kanban-trash--drop" : "",
        props.draggingTaskId ? "xhermes-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "xhermes-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "xhermes-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".xhermes-kanban-card")) return true;
      if (target.closest && target.closest(".xhermes-kanban-column-add")) return true;
      if (target.closest && target.closest(".xhermes-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("xhermes-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("xhermes-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".xhermes-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "xhermes-kanban-columns",
        isScrollable ? "xhermes-kanban-columns--scrollable" : "",
        isPanning ? "xhermes-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("xhermes-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("xhermes-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "xhermes-kanban-column",
        dragOver ? "xhermes-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "xhermes-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "xhermes-kanban-col-check",
        ...
        h("span", { className: cn("xhermes-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "xhermes-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "xhermes-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "xhermes-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "xhermes-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "xhermes-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "xhermes-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "xhermes-kanban-lane" },
                  h("div", { className: "xhermes-kanban-lane-head" },
                    h("span", { className: "xhermes-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "xhermes-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "xhermes-kanban-card--stale-red";
    if (age >= tier.amber) return "xhermes-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".xhermes-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "xhermes-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "xhermes-kanban-card",
        props.selected ? "xhermes-kanban-card--selected" : "",
        props.failed ? "xhermes-kanban-card--failed" : "",
        props.draggingSource ? "xhermes-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "xhermes-kanban-card-content" },
          h("div", { className: "xhermes-kanban-card-row" },
            h("label", {
              className: "xhermes-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "xhermes-kanban-card-check",
            ...
            h("span", { className: "xhermes-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or xhermes kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "xhermes-kanban-warning-badge",
                    "xhermes-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "xhermes-kanban-needs-assignee",
          ...
          h("div", { className: "xhermes-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "xhermes-kanban-card-row xhermes-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "xhermes-kanban-assignee",
                            title: `Assigned to XHermes profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "xhermes-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "xhermes-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "xhermes-kanban-ago",
  ...
  // centered modal reusing the xhermes-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "xhermes-kanban-dialog xhermes-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "XHermes profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "XHermes profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "xhermes-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "xhermes-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "xhermes-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "xhermes-kanban-drawer-close",
        ...
        data ? h("div", { className: "xhermes-kanban-drawer-comment-foot" },
          h("div", {
            className: "xhermes-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "xhermes-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "xhermes-kanban-section" },
      h("div", { className: "xhermes-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "xhermes-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "xhermes-kanban-drawer-body" },
      h("div", { className: "xhermes-kanban-drawer-title" },
        h("span", { className: cn("xhermes-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "xhermes-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "xhermes-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "xhermes-kanban-section" },
            h("div", { className: "xhermes-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "xhermes-kanban-section" },
            h("div", { className: "xhermes-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "xhermes-kanban-done-no-result xhermes-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "xhermes-kanban-section" },
            h("div", { className: "xhermes-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "xhermes-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "xhermes-kanban-section" },
        h("div", { className: "xhermes-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "xhermes-kanban-comment" },
            h("div", { className: "xhermes-kanban-comment-head" },
              h("span", { className: "xhermes-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "xhermes-kanban-diag-action-btn",
      ...
      h("div", { className: "xhermes-kanban-section" },
        h("div", { className: "xhermes-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "xhermes-kanban-comment" },
                h("div", { className: "xhermes-kanban-comment-head" },
                  h("span", { className: "xhermes-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "xhermes-kanban-comment-ago" },
      ...
      h("div", { className: "xhermes-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "xhermes-kanban-event",
              isDiag ? "xhermes-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "xhermes-kanban-event-header" },
                  h("span", { className: "xhermes-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "xhermes-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "xhermes-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "xhermes-kanban-event-header-plain" },
                  h("span", { className: "xhermes-kanban-event-kind" }, e.kind),
                  h("span", { className: "xhermes-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "xhermes-kanban-event-phantom-row" },
                  h("span", { className: "xhermes-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "xhermes-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "xhermes-kanban-event-payload" },
    ...
    return h("div", { className: "xhermes-kanban-section" },
      h("div", { className: "xhermes-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "xhermes-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `xhermes-kanban-run--${r.outcome || r.status || "ended"}`
          : "xhermes-kanban-run--active";
        return h("div", { key: r.id, className: cn("xhermes-kanban-run", outcomeClass) },
          h("div", { className: "xhermes-kanban-run-head" },
            h("span", { className: "xhermes-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "xhermes-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "xhermes-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "xhermes-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "xhermes-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "xhermes-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "xhermes-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "xhermes-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "xhermes-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "xhermes-kanban-pre xhermes-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "xhermes-kanban-meta-row" },
      h("span", { className: "xhermes-kanban-meta-label" }, props.label),
      h("span", { className: "xhermes-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "xhermes-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "xhermes-kanban-meta-row" },
        h("span", { className: "xhermes-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "xhermes-kanban-meta-value xhermes-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "xhermes-kanban-meta-row" },
        h("span", { className: "xhermes-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "xhermes-kanban-meta-value xhermes-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "xhermes-kanban-meta-row" },
        h("span", { className: "xhermes-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "xhermes-kanban-meta-value xhermes-kanban-editable",
    ...
    return h("div", { className: "xhermes-kanban-meta-row" },
      h("span", { className: "xhermes-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "xhermes-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "xhermes-kanban-section" },
      h("div", { className: "xhermes-kanban-section-head-row" },
        h("span", { className: "xhermes-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "xhermes-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "xhermes-kanban-section" },
      h("div", { className: "xhermes-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "xhermes-kanban-deps-row" },
        h("span", { className: "xhermes-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "xhermes-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "xhermes-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "xhermes-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "xhermes-kanban-dep-chip-x",
      ...
      h("div", { className: "xhermes-kanban-deps-row" },
        h("span", { className: "xhermes-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "xhermes-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "xhermes-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "xhermes-kanban-msg-ok"
          : "xhermes-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "xhermes-kanban-section" },
      ...
      h("div", { className: "xhermes-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "xhermes-kanban-home-sub xhermes-kanban-home-sub--on"
              : "xhermes-kanban-home-sub",
          }, label);
        })
      )
    );
