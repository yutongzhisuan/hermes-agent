"""ACP auth helpers — detect and advertise XHermes authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "xhermes-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active XHermes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if XHermes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for XHermes.
    ...
    not have XHermes provider credentials configured yet, so XHermes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate XHermes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure XHermes provider",
            description=(
                "Open XHermes' interactive model/provider setup in a terminal. "
                "Use this when XHermes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
