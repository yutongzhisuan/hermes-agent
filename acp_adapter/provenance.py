...
This is an additive XHermes extension surfaced under ACP ``_meta.xhermes`` so
...
compression rotates the internal XHermes head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.xhermes.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_hermes_session_id: The live internal XHermes DB session id
    ...
    Returns:
        A dict suitable for ``{"xhermes": {"sessionProvenance": <dict>}}`` under
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"xhermes": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"xhermes": {"sessionProvenance": prov}}
