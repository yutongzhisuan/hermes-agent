"""ACP session manager — maps ACP sessions to XHermes AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.xhermes/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when XHermes itself is running in WSL.

    Windows ACP clients can launch ``xhermes acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch XHermes from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["xhermes-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed XHermes agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by XHermes AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current XHermes runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.xhermes/state.db``) is lazily created.
        """
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the XHermes daemon's process cwd.
