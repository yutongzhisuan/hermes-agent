"""ACP agent server — exposes xHermes Agent via the Agent Client Protocol."""
...
try:
    from hermes_cli import __version__ as XHERMES_VERSION
except Exception:
    XHERMES_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so XHermes running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; XHermes cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a XHermes/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping XHermes AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show XHermes version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so XHermes maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared XHermes inventory is also used by ``xhermes model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  XHermes estimates ``used`` from the same
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.xhermes.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after XHermes changes it.

        When the internal XHermes head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hermes_session_id`` so the
        attached ``_meta.xhermes.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["xhermes-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="xhermes-agent", version=XHERMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # XHermes' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches XHermes setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.xhermes`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"xhermes": {"compactionSummary": True}}
        if kind == "merged":
            return {"xhermes": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes XHermes
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run XHermes on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive XHermes' local kawaii waiting/status
        ...
        # os.environ["XHERMES_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue XHERMES_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind XHERMES_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global XHermes workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.xhermes/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global XHERMES_CRON_SESSION (#37968).
            ...
            previous_session_id = os.environ.get("XHERMES_SESSION_ID")
            ...
            try:
                ...
                # Restore XHERMES_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("XHERMES_SESSION_ID", None)
                else:
                    os.environ["XHERMES_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal XHermes DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (XHERMES_SESSION_KEY in
        ...
        # _meta.xhermes.sessionProvenance so ACP clients can render the boundary
        ...
        # XHermes' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the XHermes install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"xHermes Agent v{XHERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when XHermes has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
