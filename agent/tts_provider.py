...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into XHermes with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.xhermes/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of XHermes
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``xhermes tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``xhermes setup`` for
