"""Upload a XHermes session transcript to Hugging Face as an agent trace.

XHermes stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``xhermes trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through XHermes' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: XHermes OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses XHermes' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render XHermes conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.xhermes/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `xhermes trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/xhermes-traces`` dataset. Returns a
