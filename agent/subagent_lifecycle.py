"""Public, plugin-safe lifecycle API for delegated XHermes subagents.
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="xhermes-lifecycle")
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active XHermes parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("XHermes failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because XHermes delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. XHermes always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
