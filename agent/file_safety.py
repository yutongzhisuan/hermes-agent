...
def _hermes_home_path() -> Path:
    """Resolve the active XHERMES_HOME (profile-aware) without circular imports."""
    try:
        from hermes_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.xhermes"))


def _hermes_root_path() -> Path:
    """Resolve the XHermes root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved XHERMES_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("XHERMES_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside XHERMES_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied XHermes path.

    Three categories are blocked:

      * Internal XHermes cache files under ``XHERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under XHERMES_HOME and the global XHermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.xhermes/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active XHERMES_HOME (profile-aware) AND the global
    # XHermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (XHERMES_HOME points at
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a XHermes credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the XHermes MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a XHermes MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied XHermes read (see
...
#
# XHermes profiles are separate XHERMES_HOME dirs under
...
#
# Reference: May 2026 incident where a xhermes-security profile session
# edited skills under both ``~/.xhermes/profiles/xhermes-security/skills/``
# AND ``~/.xhermes/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under XHERMES_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from XHERMES_HOME.

    ``~/.xhermes``              -> ``"default"``
    ``~/.xhermes/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside XHermes scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    XHermes entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to XHermes profile {info['target_profile']!r}, but the "
...
#
#   <XHERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.xhermes/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.xhermes/…`` segment and warns
# regardless of which XHermes profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.xhermes/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.xhermes`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.xhermes/…`` and returns the
    index where the inner XHermes-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .xhermes / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".xhermes":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative XHermes state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.xhermes``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.xhermes`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any XHermes resolver to
    succeed, so it works correctly even when called from contexts where
    XHERMES_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host XHermes process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real XHERMES_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.xhermes/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.xhermes/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host XHERMES_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative XHermes state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host XHermes "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real XHERMES_HOME. Use the "
