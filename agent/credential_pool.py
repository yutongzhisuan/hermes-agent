...
# un-throttled log fires on *every* selection. On Windows several XHermes
...
# readiness handshake ("Timed out connecting to XHermes backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``xhermes model`` / ``xhermes auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another XHermes process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``xhermes`` invocation that defaults to the active provider
        (e.g. setup wizard, ``xhermes auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across XHermes
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another XHermes
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another XHermes process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other XHermes profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `xhermes model` / `xhermes auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `xhermes model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``xhermes auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `xhermes auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `xhermes auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, XHermes PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `xhermes setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.xhermes/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `xhermes setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `xhermes auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (xhermes auth remove copilot gh_cli) must not pay either on
        ...
        # XHermes auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.xhermes/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `xhermes auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `xhermes auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the XHermes auth store.  Without this gate the removal is instantly
        ...
        # XHermes owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``xhermes model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``xhermes auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.xhermes/.env over os.environ — the user's config file is the
    # authoritative source for XHermes credentials. Stale env vars from parent
    ...
    # Honour user suppression — `xhermes auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.xhermes/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.xhermes/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `xhermes auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and XHermes
