...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  XHermes pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<hermes_home>/proxy/ca.{crt,key}`` is generated on
  first ``xhermes egress setup``.  Sandboxes trust this CA so iron-proxy can
...
* The proxy runs as a managed subprocess (``xhermes egress start``), pidfile
...
# request needs the bearer key below.  ``xhermes egress reload`` is the
# client.
#
...
# credential (XHermes' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.xhermes/proxy/`` just to
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``xhermes proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="xhermes-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``xhermes egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="xhermes-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "xhermes-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="xhermes-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `xhermes egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `xhermes egress setup` and use "
            "`xhermes egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `xhermes egress setup`, "
            "then `xhermes egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``xhermes egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `xhermes egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a XHermes dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``xhermes egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``xhermes egress setup`` should not invalidate the tokens
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `xhermes egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `xhermes egress setup` first."
        )
    ...
    # close-on-success path, every restart leaked one fd in the XHermes
    # process.
    #
    ...
    # ``xhermes egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``xhermes egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `xhermes egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`xhermes egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `xhermes secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
