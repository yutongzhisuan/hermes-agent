...
Data resolution order:
  ...
  2. Disk cache (~/.xhermes/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: XHermes ↔ models.dev
# ---------------------------------------------------------------------------

# XHermes provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → XHermes (built lazily)
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``xhermes config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a XHermes provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal XHermes agent traffic.
...
def get_provider_info(
    ...
    Accepts either a XHermes provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve XHermes ID → models.dev ID
...
def get_model_info(
    ...
    Accepts XHermes or models.dev provider ID.  Tries exact match then
