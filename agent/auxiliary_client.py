...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.xhermes/auth.json active provider)
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `xhermes update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # XHermes sees a single failure (issue #54465). Disable SDK-internal retries
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="xhermes-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before XHermes triggers summarization.  Higher values delay
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (xHermes Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private XHermes plumbing (_reasoning_config
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.xhermes/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``xhermes auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from XHermes auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `xhermes setup` or `xhermes model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second xhermes
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``xhermes config aux reset``)."""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into XHermes' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `xhermes model` but the
    #    old OPENAI_BASE_URL lingers in ~/.xhermes/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: xhermes model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.xhermes/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so XHermes owns
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: xhermes auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: xhermes model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: xhermes model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: xhermes doctor for "
                "diagnostics)"
            )
            return None, None
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate XHermes reasoning into native
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional XHermes reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `xhermes model`."
                    )
