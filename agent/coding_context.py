"""Coding-context awareness — base XHermes, every interactive surface.

When the user runs XHermes inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), XHermes shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("xhermes.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "xhermes", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
def is_coding_context(
    *,
    ...
    """Whether XHermes should operate in its coding posture right now."""
