...
(``gateway/platforms/webhook.py``): inbound wakes XHermes when the world
changes; outbound tells the world when XHermes does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-XHermes-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Config schema (``~/.xhermes/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/xhermes-events
          ...
          name: ci-notify   # optional label for logs / `xhermes hooks list`
...
Headers::
    ...
    User-Agent:              XHermes-Agent-Outbound-Webhook
    X-XHermes-Event:          <hook event name>
    X-XHermes-Delivery:       <delivery_id>
    X-XHermes-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-XHermes-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-XHermes-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
