...
row taxonomy is inferred from the atlas shape so XHermes can render both legacy
...
# rows (1536x1872), while older XHermes/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are XHermes' activity state names. They are not always identical to the
...
# Legacy XHermes/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical XHermes activity names -> accepted row-name aliases in descending
