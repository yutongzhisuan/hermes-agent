...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time XHermes starts and finds ``~/.openclaw/``.

    Points users at ``xhermes claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``xhermes claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to XHermes, run "
        "`xhermes claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `xhermes claw cleanup` (renames it to ~/.openclaw.pre-migration — "
