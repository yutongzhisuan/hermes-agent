...
Owns one Codex thread per XHermes session. Drives `turn/start`, consumes
...
# XHermes' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_XHERMES_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse XHermes/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per XHermes session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _XHERMES_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("XHERMES_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but XHermes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into XHermes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own xhermes-tools callback we
            # auto-accept — the user already approved XHermes' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO XHermes
        approval-mode/timeout logic. The XHermes-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; XHermes approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map XHermes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    XHermes returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the XHermes approval mode/timeout resolution
    ...
    # "prompt expired"; the XHermes-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort XHermes version string for codex's userAgent line."""
    try:
