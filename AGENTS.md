# XHermes Agent - Development Guide

Instructions for AI coding assistants and developers working on the xhermes-agent codebase.

**Never give up on the right solution.**

## What XHermes Is

XHermes is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: XHermes ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`xhermes tools`, `xhermes setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `XHERMES_HOME`. Mocks hide integration bugs.
...
- **New `XHERMES_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `xhermes tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.xhermes/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `xhermes <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `xhermes webhook`, `xhermes cron`, `xhermes tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.xhermes/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.xhermes/xhermes-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _XHERMES_CORE_TOOLS list
...
│   ├── xhermes-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `xhermes --tui`
...
**User config:** `~/.xhermes/config.yaml` (settings), `~/.xhermes/.env` (API keys only).
**Logs:** `~/.xhermes/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `xhermes logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across XHermes: desktop, TUI, website, and future TS packages.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.xhermes/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
- **Slack** — `slack_subcommand_map()` generates `/xhermes` subcommand routing
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `xhermes --tui` or `XHERMES_TUI=1`.
...
npm run dev       # watch mode (rebuilds xhermes-ink + tsx --watch)
npm start         # production
npm run build     # full build (xhermes-ink + tsc)
...
### TUI in the Dashboard (`xhermes dashboard` → `/chat`)

The dashboard embeds the real `xhermes --tui` — **not** a rewrite.  See `hermes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `hermes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `xhermes --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `xhermes --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@xhermes/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `xhermes serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `XHERMES_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `xhermes` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `xhermes --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
be core tools. For custom or local-only tools, do **not** edit XHermes core.
Use the plugin route instead: create `~/.xhermes/plugins/<name>/plugin.yaml`
and `~/.xhermes/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core XHermes tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_XHERMES_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_XHERMES_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_hermes_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `XHERMES_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".xhermes"`. This ensures each profile gets its own state.
...
| `load_config()` | `xhermes tools`, `xhermes setup`, most CLI subcommands | `hermes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
~/.xhermes/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Users create `~/.xhermes/skins/<name>.yaml`:
...
XHermes has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.xhermes/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.xhermes/plugins/`, `./.xhermes/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `xhermes` at startup so
  `xhermes <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `xhermes <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `xhermes --help`.
...
into `~/.xhermes/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `xhermes memory setup` / `post_setup()` without
...
`~/.xhermes/plugins/` (or via pip entry points). They register through
...
2. User: `$XHERMES_HOME/plugins/model-providers/<name>/`
...
[`xhermes-example-plugins`](https://github.com/NousResearch/xhermes-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `xhermes skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.xhermes.tags`, `metadata.xhermes.category`,
`metadata.xhermes.related_skills`, `metadata.xhermes.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.xhermes.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native XHermes tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "XHermes Agent" is the secondary collaborator. If the
   contributor's commit shows "XHermes Agent" as author (because they
   used XHermes to draft the skill), replace it with their actual name
...
lives in the `xhermes-agent-dev` skill at
...
`"messaging"`); `_XHERMES_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `xhermes tools` (the curses UI) or the
...
XHermes returns a delegation id immediately and the result re-enters the
...
go to `~/.xhermes/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `hermes_cli/curator.py` wires `xhermes curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.xhermes/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `xhermes cron <verb>`
...
- File lock at `~/.xhermes/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `xhermes kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `hermes_cli/kanban.py` wires `xhermes kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`xhermes-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `XHERMES_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
XHermes-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `XHERMES_BACKGROUND_NOTIFICATIONS` env var):
...
XHermes supports **profiles** — multiple fully isolated instances, each with its own
`XHERMES_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `hermes_cli/main.py` sets
`XHERMES_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all XHERMES_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.xhermes` or `Path.home() / ".xhermes"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".xhermes" / "config.yaml"
   ```

2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.xhermes` for default or `~/.xhermes/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.xhermes/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_hermes_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_hermes_home()`,
   not `Path.home() / ".xhermes"`.

4. **Tests that mock `Path.home()` must also set `XHERMES_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".xhermes"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"XHERMES_HOME": str(tmp_path / ".xhermes")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not XHERMES_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".xhermes" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
   This is intentional — it lets `xhermes -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.xhermes` paths
Use `get_hermes_home()` from `hermes_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.xhermes` breaks profiles — each profile
has its own `XHERMES_HOME` directory. This was the source of 5 bugs fixed in PR #3575.
...
with actual imports (not mocks) against a temp `XHERMES_HOME`.

### Tests must not write to `~/.xhermes/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `XHERMES_HOME` to a temp dir. Never hardcode `~/.xhermes/` paths in tests.
...
subprocess (`--file-retries`, default 1; `XHERMES_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.xhermes/` | Your real config+auth.json                  | Temp dir per test                         |
