...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``xhermes://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``xhermes://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"xhermes://blueprint/{quote(blueprint.key)}{qs}"
