...
One ``CDPSupervisor`` runs per XHermes ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach XHermes logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "xhermes-dialog-bridge.invalid"
...
(() => {
  ...
  const ENDPOINT = "http://xhermes-dialog-bridge.invalid/";
