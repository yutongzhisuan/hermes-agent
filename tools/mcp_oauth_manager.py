...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``xhermes mcp reauth``.
...
_XHERMES_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _XHERMES_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `xhermes mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``xhermes mcp remove <name>`` and (indirectly) by
        ``xhermes mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
