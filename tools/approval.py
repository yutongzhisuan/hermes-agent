...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("XHERMES_YOLO_MODE", ""))
...
# os.environ["XHERMES_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export XHERMES_INTERACTIVE.
...
def set_hermes_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["XHERMES_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``XHERMES_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``XHERMES_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hermes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("XHERMES_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("XHERMES_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("XHERMES_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("XHERMES_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("XHERMES_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set XHERMES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind XHERMES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds XHERMES_SESSION_PLATFORM via
...
# via shell expansions like $HOME or $XHERMES_HOME, or by the resolved absolute
# active profile home path such as /home/xhermes/.xhermes/config.yaml. The
# resolved-absolute form is folded into the ~/.xhermes/ patterns at detection
...
# go stale when XHERMES_HOME is set after this module is imported, e.g. under the
...
# ~/.xhermes/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _XHERMES_ENV_PATH; matches the XHERMES_HOME override form as
# well as ~/.xhermes/.
_XHERMES_CONFIG_PATH = (
    r'(?:~\/\.xhermes/|'
    r'(?:\$home|\$\{home\})/\.xhermes/|'
    r'(?:\$hermes_home|\$\{hermes_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_XHERMES_ENV_PATH}|'
    rf'{_XHERMES_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by XHermes: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `xhermes` and `gateway` (e.g. `xhermes -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart xhermes gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "xhermes update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `xhermes gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `xhermes ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart xhermes-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart xhermes-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(xhermes|gateway|cli\.py)\b', "kill xhermes/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill xhermes` but not
    # `kill -9 $(pgrep -f xhermes)` because the substitution is opaque
    ...
    # the `xhermes gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.xhermes.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(xhermes|ai\.xhermes)\b', "stop/restart xhermes launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/XHermes file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.xhermes/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a XHermes-managed security file (~/.xhermes/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_XHERMES_CONFIG_PATH}|{_XHERMES_ENV_PATH})', "in-place edit of XHermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_XHERMES_CONFIG_PATH}|{_XHERMES_ENV_PATH})', "in-place edit of XHermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_XHERMES_CONFIG_PATH}|{_XHERMES_ENV_PATH})', "in-place edit of XHermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.xhermes/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / XHERMES_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) XHermes home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\xhermes), so folding the user home
    # first would eat the prefix the XHermes-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / XHERMES_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.xhermes``); the matched
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute XHermes home prefix to ``~/.xhermes/``.

    Resolves the active ``XHERMES_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.xhermes/`` so the static ``_XHERMES_CONFIG_PATH`` / ``_XHERMES_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/xhermes/.xhermes/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$XHERMES_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.xhermes")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``xhermes send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one XHermes ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"xhermes-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from XHermes YOLO.
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["XHERMES_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "XHERMES_SPINNER_PAUSE" in os.environ:
            del os.environ["XHERMES_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses XHermes approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``XHERMES_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            XHERMES_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set XHERMES_INTERACTIVE or "
                "XHERMES_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set XHERMES_INTERACTIVE or "
            "XHERMES_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("XHERMES_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``XHERMES_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("XHERMES_EXEC_ASK")
