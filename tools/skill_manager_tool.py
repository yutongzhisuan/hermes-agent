...
~/.xhermes/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `xhermes config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.xhermes/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch XHERMES_HOME and
    ...
    live profile-scoped XHERMES_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`xhermes curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`xhermes curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `xhermes curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `xhermes curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.xhermes/skills/) first, then any
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `xhermes sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `xhermes sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `xhermes sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `xhermes sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER XHermes profiles.
    ...
    # Default profile (~/.xhermes/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.xhermes/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`xhermes -p "
            ...
            base += (
                ...
                f"Switch profiles (`xhermes -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.xhermes/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `xhermes curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `xhermes curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "Create when a complex task succeeded, errors were overcome, a "
        "user-corrected approach worked, a non-trivial workflow turned up, or "
        "the user asks you to remember a procedure; skip simple one-offs. "
        "Update when instructions go stale or wrong, on OS-specific failures, "
        "or when use reveals missing steps or pitfalls — patch immediately "
        "rather than working around it. Confirm with the user before creating "
        "or deleting.\n\n"
        "Good skills: trigger conditions, numbered steps with exact commands, "
        "pitfalls section, verification steps. Use skill_view() to see format "
        "examples. The frontmatter description is truncated to the first 57 "
        "chars plus '...' in the system prompt skill index (full text stays "
        "visible via skills_list/skill_view), so keep the trigger self-contained "
        "in that window: 'Use when <trigger>. <one-line behavior>.'\n\n"
        "Pinned skills refuse delete only; patch and edit still apply to them."
    ),
    "parameters": {
        "type": "object",
        "properties": {
