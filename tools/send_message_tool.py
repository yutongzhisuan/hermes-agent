    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("XHERMES_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("XHERMES_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("XHERMES_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("XHERMES_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``XHERMES_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``XHERMES_HOME`` which has no ``gateway.pid``, so the gateway-running check
