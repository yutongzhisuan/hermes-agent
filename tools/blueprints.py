...
`xhermes skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.xhermes.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.xhermes.blueprint`` is a mapping containing
    ...
    xhermes = meta.get("xhermes") if isinstance(meta, dict) else None
    blueprint = xhermes.get("blueprint") if isinstance(xhermes, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.xhermes.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.xhermes.blueprint`` block) they can hand
    to ``xhermes skills publish`` to share. ``body`` is the plain-language
