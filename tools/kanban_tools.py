...
Why tools instead of just shelling out to ``xhermes kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``xhermes kanban complete …``
   inside the container, where ``xhermes`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.xhermes/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``xhermes kanban …``), the dashboard
(``xhermes dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``xhermes chat`` without the kanban toolset see zero
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``xhermes-system`` and poison
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; XHermes downloads it (with the shared size cap)
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — XHermes downloads it server-side "
