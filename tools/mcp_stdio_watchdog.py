...
<url>``) is spawned as a direct child of the XHermes process. XHermes's own
...
XHermes process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next XHermes startup's opt-in
