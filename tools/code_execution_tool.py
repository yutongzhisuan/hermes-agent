...
Lets the LLM write a Python script that calls XHermes tools via RPC,
...
# available on every platform XHermes itself runs on.
...
# match a safe prefix, the operational XHERMES_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports XHermes
# code can lose the DB-layer Kanban mutation guard while still inheriting
# XHERMES_HOME.
#
# NB: the broad "XHERMES_" prefix was deliberately removed (#27303) — it leaked
# XHERMES_*-named config that lacks a secret substring (e.g. XHERMES_BASE_URL,
# XHERMES_KANBAN_DB, XHERMES_*_WEBHOOK).  The child only needs the few
# location/profile vars in _XHERMES_CHILD_ALLOWED below; XHERMES_RPC_SOCKET /
# XHERMES_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational XHERMES_* vars the child legitimately needs by exact name — these
...
_XHERMES_CHILD_ALLOWED = frozenset({
    "XHERMES_HOME",
    "XHERMES_PROFILE",
    "XHERMES_CONFIG",
    "XHERMES_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational XHERMES_* vars (_XHERMES_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret XHERMES_* vars dropped by the tightened allowlist (#27303). The
    # broad "XHERMES_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like XHERMES_KANBAN_DB / XHERMES_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _XHERMES_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("XHERMES_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped XHERMES_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted XHERMES_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-XHermes-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-XHermes-profile soft guard."""',
...
"""Auto-generated XHermes tools RPC stubs."""
...
def _connect():
    ...
    XHERMES_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["XHERMES_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("XHERMES_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated XHermes tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("XHERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"XHERMES_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"XHERMES_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("XHERMES_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from XHERMES_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  XHERMES_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["XHERMES_RPC_SOCKET"] = rpc_endpoint
        child_env["XHERMES_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the xhermes-agent root is importable in the sandbox so
        ...
        # XHERMES_TIMEZONE is an internal XHermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("XHERMES_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("XHERMES_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.xhermes/.env')).
...
def _load_config() -> dict:
    ...
    ``xhermes --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (xhermes-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``xhermes tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and xhermes-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.xhermes/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls XHermes tools programmatically. "
