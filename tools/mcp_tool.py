...
transport, discovers their tools, and registers them into the xhermes-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.xhermes/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.xhermes/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach XHermes' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _jittered(seconds: float) -> float:
    ...
    return max(
        0.0, seconds * random.uniform(1.0 - _BACKOFF_JITTER, 1.0 + _BACKOFF_JITTER)
    )
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    XHermes explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so XHermes and
...
async def _paginate_full_list(list_method, items_attr: str, server_name: str):
    ...
    for _ in range(_MCP_LIST_MAX_PAGES):
        ...
        logger.warning(
            "MCP server '%s': %s pagination exceeded %d pages; truncating at %d items",
            server_name,
            items_attr,
            _MCP_LIST_MAX_PAGES,
            len(items),
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv("XHERMES_HOME", str(get_hermes_home()))
            )
            candidates = [
                ...
                # image (which the XHermes Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → XHermes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that XHermes gateways know how to render.
...
def _cache_mcp_audio_block(block) -> str:
    ...
    data = getattr(block, "data", None)
    mime_type = (
        str(getattr(block, "mimeType", None) or "").split(";", 1)[0].strip().lower()
    )
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the XHermes document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through XHermes' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (XHERMES_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit XHERMES_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into XHermes' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `xhermes setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this XHermes process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through XHermes' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until XHermes is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `xhermes mcp login {server_name}` (or delete the tokens "
        f"file under ~/.xhermes/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent XHermes processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local XHERMES_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local XHERMES_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the XHermes config file.
    ...
    ``os.environ`` (which includes ``~/.xhermes/.env`` loaded at startup).
    """
    try:
        from hermes_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("XHERMES_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through XHermes' MEDIA: tag convention and out to messaging
            ...
            # XHermes' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves XHermes's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. XHermes uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the XHermes registry schema format.
...
async def _discover_and_register_server(name: str, config: dict) -> List[str]:
    ...
    try:
        ...
        task_cancelling = (
            task.cancelling() if task is not None and hasattr(task, "cancelling") else 0
        )
        if (
            server is not None
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    with _lock:
        connected = [
            n for n in new_servers if n in _servers and n not in _server_connect_errors
        ]
        new_tool_count = sum(
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``xhermes tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the XHermes registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple xhermes
    ...
    with _lock:
        ...
        if include_active:
            ...
            if server_name is not None:
                active = {
                    pid: owner for pid, owner in active.items() if owner == server_name
                }
            pids.update(active)
            for pid in active:
