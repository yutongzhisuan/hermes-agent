...
# 5. Symlinks the 'xhermes' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ XHermes Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/xhermes-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink xhermes into a user-facing bin dir
# ============================================================================
...
echo "Other commands:"
echo "  xhermes status        # Check configuration"
if is_termux; then
    echo "  xhermes gateway       # Run gateway in foreground"
else
    echo "  xhermes gateway install # Install gateway service (messaging + cron)"
fi
echo "  xhermes cron list     # View scheduled jobs"
echo "  xhermes doctor        # Diagnose issues"
echo ""
