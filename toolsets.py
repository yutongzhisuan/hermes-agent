...
_XHERMES_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on XHERMES_DESKTOP
    ...
    # spawned as a kanban worker (XHERMES_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `xhermes tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``xhermes tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (XHERMES_KANBAN_TASK env "
    ...
    # Coding posture (base XHermes — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # ==========================================================================
    # Full XHermes toolsets (CLI + messaging platforms)
    #
    ...
    # the `xhermes send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "xhermes-cron": {
        # Mirrors xhermes-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `xhermes tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as xhermes-cli; gated by `xhermes tools`",
        "tools": _XHERMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "xhermes-email": {
        "description": "Email bot toolset - interact with XHermes via email (IMAP/SMTP)",
        "tools": _XHERMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "xhermes-sms": {
        "description": "SMS bot toolset - interact with XHermes via SMS (Twilio)",
        "tools": _XHERMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "xhermes-webhook": {
        ...
        "tools": _XHERMES_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "xhermes-gateway": {
        ...
        "tools": [],
        "includes": ["xhermes-telegram", "xhermes-discord", "xhermes-whatsapp", "xhermes-slack", "xhermes-signal", "xhermes-bluebubbles", "xhermes-homeassistant", "xhermes-email", "xhermes-sms", "xhermes-mattermost", "xhermes-matrix", "xhermes-dingtalk", "xhermes-feishu", "xhermes-wecom", "xhermes-wecom-callback", "xhermes-weixin", "xhermes-qqbot", "xhermes-webhook", "xhermes-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``xhermes-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_XHERMES_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``xhermes-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (xhermes-<name>).
        # Gives them _XHERMES_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("xhermes-"):
            platform_name = name[len("xhermes-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_XHERMES_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
