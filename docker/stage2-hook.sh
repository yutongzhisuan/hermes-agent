...
#
# Wired into the image as /etc/cont-init.d/01-xhermes-setup by the
...
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid xhermes "$@"; }
...
# `id -u xhermes` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# xhermes UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u xhermes)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-xhermes UID).
...
and the baked /opt/xhermes install tree is intentionally root-owned and
...
# host's `docker` group (or root). The supervised xhermes user (UID 10000)
...
# xhermes process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes xhermes. Idempotent across container
...
#   - xhermes is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G xhermes 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] xhermes already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" xhermes 2>/dev/null; then
        echo "[stage2] Added xhermes to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group xhermes failed; docker backend may fail with EACCES"
    fi
    break
done
...
#
# The canonical list of xhermes-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hermes_uid=$(id -u xhermes)
...
chown_hermes_tree() {
    ...
    chown -R xhermes:xhermes "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_hermes_owner() {
    target="$1"
    find "$target" \( ! -user xhermes -o ! -group xhermes \) -print -quit 2>/dev/null | grep -q .
}
...
# reconciler (02-reconcile-profiles) which runs as xhermes and walks
...
# --- Seed directory structure as xhermes user ---
# Run as xhermes via s6-setuidgid so dirs end up owned correctly (matters
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/xhermes/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI XHermes spawns for the
...
# main-wrapper.sh propagates it into the supervised ``xhermes`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-xhermes, dashboard, etc.).
