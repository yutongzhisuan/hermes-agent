...
#
# Test hook: XHERMES_TINI_SHIM_TARGET overrides the `/init` path (and
# XHERMES_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${XHERMES_TINI_SHIM_TARGET:-/init}"
WRAPPER="${XHERMES_TINI_SHIM_WRAPPER:-/opt/xhermes/docker/main-wrapper.sh}"
