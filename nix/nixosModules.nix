# nix/nixosModules.nix — NixOS module for xhermes-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.xhermes-agent;
    effectivePackage =
    ...
    xhermes-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-xhermes-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "xhermes-config-attrs";
      description = "XHermes YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — xhermes
    ...
    generatedConfigFile = pkgs.writeText "xhermes-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "xhermes-documents" { } (
      ''
        mkdir -p $out
    ...
    # xhermes user + sudo on first boot (writable layer persists), then
    ...
    containerEntrypoint = pkgs.writeShellScript "xhermes-container-entrypoint" ''
      ...
      if [ ! -f /var/lib/xhermes-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/xhermes-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/xhermes ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/xhermes
        chmod 0440 /etc/sudoers.d/xhermes
      fi
    ...
    # Default: /var/lib/xhermes/workspace → /data/workspace.
  ...
  in {
    options.services.xhermes-agent = with lib; {
      enable = mkEnableOption "XHermes Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = xhermes-agent;
        description = "The xhermes-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative XHermes config (attrset). Deep-merged across module
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `xhermes gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the xhermes user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the xhermes plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. XHermes discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as xhermes (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in xhermes-agent's
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.xhermes symlink to the service
            stateDir. These users are automatically added to the xhermes group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.xhermes-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.xhermes-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the xhermes user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.xhermes-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed xhermes
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure xhermes is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.xhermes        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xhermes/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xhermes/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xhermes/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xhermes/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xhermes/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."xhermes-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.xhermes
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.xhermes ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.xhermes ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.xhermes -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.xhermes/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.xhermes/$_subdir"
            chmod 2770 "${cfg.stateDir}/.xhermes/$_subdir"
            find "${cfg.stateDir}/.xhermes/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # xhermes-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.xhermes/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.xhermes/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xhermes/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.xhermes/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.xhermes/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xhermes/.managed
          chmod 0644 ${cfg.stateDir}/.xhermes/.managed
    ...
    XHERMES_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.xhermes/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xhermes";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.xhermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "xhermes-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.xhermes -> stateDir/.xhermes for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xhermes";
                target = "${cfg.stateDir}/.xhermes";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "xhermes-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xhermes/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.xhermes/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xhermes/auth.json
              fi
            ''}
          ''}
    ...
    XHERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.xhermes/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.xhermes/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xhermes/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.xhermes-agent = {
          description = "XHermes Agent Gateway";
          ...
          serviceConfig = {
            ...
            # so interactive users in the xhermes group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.xhermes-agent = {
          description = "XHermes Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/xhermes gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
