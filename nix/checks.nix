...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      xhermes-agent = self'.packages.default;
      hermesVenv = xhermes-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "xhermes-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "xhermes-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "xhermes-build-package" { } ''
          echo "PASS: package built at ${xhermes-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "xhermes-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "xhermes-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${xhermes-agent}/bin/xhermes || (echo "FAIL: xhermes binary missing"; exit 1)
          test -x ${xhermes-agent}/bin/xhermes-agent || (echo "FAIL: xhermes-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${xhermes-agent}/bin/xhermes version 2>&1 | grep -qi "xhermes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "xhermes-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in xhermes xhermes-agent xhermes-acp; do
            test -x ${xhermes-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "xhermes-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking xhermes --help ==="
          ${xhermes-agent}/bin/xhermes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${xhermes-agent}/bin/xhermes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "xhermes-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${xhermes-agent}/share/xhermes-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${xhermes-agent}/share/xhermes-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "XHERMES_BUNDLED_SKILLS" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: XHERMES_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${xhermes-agent}/share/xhermes-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${xhermes-agent}/share/xhermes-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "XHERMES_OPTIONAL_SKILLS" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, XHERMES_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "xhermes-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${xhermes-agent}/share/xhermes-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${xhermes-agent}/share/xhermes-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "XHERMES_BUNDLED_PLUGINS" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: XHERMES_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "xhermes-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${xhermes-agent}/share/xhermes-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${xhermes-agent}/share/xhermes-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${xhermes-agent}/share/xhermes-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "XHERMES_BUNDLED_LOCALES" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: XHERMES_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + XHERMES_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && XHERMES_BUNDLED_LOCALES=${xhermes-agent}/share/xhermes-agent/locales \
            ${hermesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with XHERMES_BUNDLED_LOCALES set"; exit 1)
        ...
        # XHERMES_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "xhermes-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${xhermes-agent}/share/xhermes-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${xhermes-agent}/share/xhermes-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "XHERMES_OPTIONAL_MCPS" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: XHERMES_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${xhermes-agent}/bin/xhermes mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: xhermes mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "xhermes-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${xhermes-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${xhermes-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "XHERMES_TUI_DIR" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: XHERMES_TUI_DIR set in wrapper"
        ...
        # Verify XHERMES_NODE is set in wrapper and points to Node 26+
        # (XHermes pins its toolchain to Node 26 everywhere)
        xhermes-node = pkgs.runCommand "xhermes-node-version" { } ''
          set -e
          echo "=== Checking XHERMES_NODE in wrapper ==="
          grep -q "XHERMES_NODE" ${xhermes-agent}/bin/xhermes || \
            (echo "FAIL: XHERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: XHERMES_NODE present in wrapper"

          XHERMES_NODE=$(sed -n "s/^export XHERMES_NODE='\(.*\)'/\1/p" ${xhermes-agent}/bin/xhermes)
          test -x "$XHERMES_NODE" || (echo "FAIL: XHERMES_NODE=$XHERMES_NODE not executable"; exit 1)
          echo "PASS: XHERMES_NODE executable at $XHERMES_NODE"

          NODE_MAJOR=$("$XHERMES_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, XHermes requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All XHERMES_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify XHERMES_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "xhermes-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(XHERMES_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking XHERMES_MANAGED guards ==="
          check_blocked "config set" ${xhermes-agent}/bin/xhermes config set model foo
          check_blocked "config edit" ${xhermes-agent}/bin/xhermes config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = xhermes-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "xhermes-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/xhermes || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/xhermes || \
          ...
          if grep -q "PYTHONPATH" ${xhermes-agent}/bin/xhermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = xhermes-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "xhermes-extra-dependency-groups" { } ''
          set -e
        ...
        messaging-variant = pkgs.runCommand "xhermes-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "xhermes-config-roundtrip" {
          ...
          merge_and_load() {
            local hermes_home="$1"
            export XHERMES_HOME="$hermes_home"
