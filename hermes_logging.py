"""Centralized logging setup for xHermes Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.xhermes/logs/`` (profile-aware via ``get_hermes_home()``).
...
# essentially always in XHermes (TUI, gateway, ``hy_memory`` server, MCP
...
# Used by _ComponentFilter and exposed for ``xhermes logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``xhermes logs --component gateway``.
...
def setup_logging(
    *,
    ...
    """Configure the XHermes logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the XHermes home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the xhermes group. However, both ``_open()`` (initial creation)
...
# the module header): when several XHermes processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="xhermes-log-drain", daemon=True)
    t.start()
    t.join(timeout)
