...
manual BotFather token copy-paste. XHermes talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.xhermes-agent.nousresearch.com"
...
DEFAULT_BOT_NAME = "xHermes Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting XHermes Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the XHermes Telegram onboarding service.")
