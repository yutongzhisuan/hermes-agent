"""``xhermes gateway`` and ``xhermes proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if XHERMES_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy xhermes.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy XHermes gateway unit files "
            "(e.g. xhermes.service) left over from older installs. Profile "
            "units (xhermes-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.xhermes/.env. "
            "Requires being logged in (xhermes setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.xhermes/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `xhermes proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
