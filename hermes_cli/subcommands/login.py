"""``xhermes login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``xhermes login`` was removed in favor of ``xhermes auth`` / ``xhermes model``
    ...
    old scripts/aliases invoking ``xhermes login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``xhermes --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``xhermes login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``xhermes model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `xhermes auth` to manage credentials, "
            "`xhermes model` to select a provider, or `xhermes setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `xhermes model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: xhermes-cli)"
    )
