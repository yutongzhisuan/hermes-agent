...
the tool registry or launching XHermes (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``xhermes-cli`` to the non-existent ``xhermes``. ``resolve_toolset('xhermes')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``xhermes-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``xhermes`` instead of ``xhermes-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `xhermes tools` to reconfigure."
        )
    return warnings
