"""`xhermes checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.xhermes/checkpoints/``.  Actions:

    xhermes checkpoints               # same as `status`
    xhermes checkpoints status        # total size, project count, breakdown
    xhermes checkpoints list          # per-project checkpoint counts + workdir
    xhermes checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    xhermes checkpoints clear [-f]    # nuke the entire base (asks first)
    xhermes checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    xhermes checkpoints
    xhermes checkpoints prune --retention-days 3 --max-size-mb 200
    xhermes checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: xhermes checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``xhermes checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `xhermes checkpoints` → status
