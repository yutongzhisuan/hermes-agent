"""``xhermes approvals suggest`` — mine approval history into allowlist proposals.

XHermes has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.xhermes/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, xhermes self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/xhermes home prefixes and collapse whitespace.
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  xhermes approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.xhermes/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``xhermes approvals suggest``."""
