"""
XHermes Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with xhermes-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.xhermes/plugins/<name>/``
3. **Project plugins** – ``./.xhermes/plugins/<name>/`` (opt-in via
   ``XHERMES_ENABLE_PROJECT_PLUGINS``)
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``XHERMES_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("XHERMES_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``XHERMES_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.xhermes/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("XHERMES_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When XHERMES_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other XHermes loggers.
    """
    ...
    logger.debug(
        "XHERMES_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. XHermes' shipped guidance lives in the evidence-based
    ...
    # separate `xhermes -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `xhermes kanban dispatch`),
...
class PluginManifest:
    ...
    #              in ~/.xhermes/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``xhermes plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active XHermes profile name (e.g. ``"default"``).

        Derived from ``XHERMES_HOME`` via
        ...
        running under ``~/.xhermes/profiles/<name>``, or ``"custom"`` when
        ``XHERMES_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with XHermes core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``xhermes honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``xhermes <subcommand>``
    ...
    def register_secret_source(self, source) -> None:
        ...
        ``~/.xhermes/.env`` loads, before XHermes reads credentials — when
        ...
        spawned XHermes process (gateway children, cron sessions,
    ...
    def register_slack_action_handler(
        ...
        XHermes' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``xhermes model → Configure auxiliary models`` picker
    ...
    def register_skill(
        ...
        ``~/.xhermes/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("XHERMES_SAFE_MODE"):
            logger.info("XHERMES_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 2. User plugins (~/.xhermes/plugins/)
        ...
        # 3. Project plugins (./.xhermes/plugins/)
        if _env_enabled("XHERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".xhermes" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set XHERMES_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with xhermes and must
            ...
            # all ~20 of them added several seconds to every `xhermes`
            # invocation — including plain `xhermes chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform XHermes ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `xhermes plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``xhermes plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`xhermes plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `xhermes plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="xhermes-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``xhermes tools`` TUI so plugin-provided toolsets appear
