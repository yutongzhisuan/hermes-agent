...
The provider list shown by ``xhermes model`` (CLI/TUI) and the desktop Settings
...
the same universe ``xhermes model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``xhermes model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `xhermes model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``xhermes model`` universe.
    ...
    # XHermes overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = XHERMES_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # XHermes overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
