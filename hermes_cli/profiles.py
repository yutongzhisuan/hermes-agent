...
The "default" profile is ``~/.xhermes`` itself — backward compatible,
zero migration needed.

Usage::

    xhermes profile create coder          # fresh profile + bundled skills
    xhermes profile create coder --clone  # also copy config, .env, SOUL.md, skills
    xhermes profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    xhermes -p coder chat                 # or via flag
    xhermes profile use coder             # set as sticky default
    xhermes profile delete coder          # remove profile + alias + service
"""
...
# default profile (``~/.xhermes``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   xhermes-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `xhermes backup` archives
...
# Marker file written by `xhermes profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `xhermes
...
# `xhermes skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         XHermes infrastructure directories that only the default profile
         (``~/.xhermes``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.xhermes) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "xhermes-agent",  # repo checkout (multi-GB)
    ".worktrees",  # git worktrees
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the XHermes installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, xhermes subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running XHermes. If we
    ...
    try:
        ...
        # not be migrated. The next `xhermes doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``xhermes -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``xhermes profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``xhermes -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``xhermes profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing XHermes config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``xhermes update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.xhermes)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.xhermes/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `xhermes -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this XHermes profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `xhermes update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`xhermes profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running XHermes (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `xhermes profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `xhermes -p <profile> gateway start` can supervise it via
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running XHermes *backends* bound to this profile.
    ...
    # Never terminate ourselves or a parent (e.g. `xhermes -p <canon> profile
    ...
    hermes_markers = ("hermes_cli.main", "xhermes-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a XHermes process: either an entrypoint marker in argv, or
            # a resolved executable named `xhermes`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_hermes = (
                any(marker in joined for marker in hermes_markers)
                or exe_name == "xhermes"
                or exe_name.startswith("xhermes")
            )
            if not is_hermes:
                continue
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.xhermes).\n"
            "To remove everything, use: xhermes uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.xhermes/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: xhermes profile create {canon}"
        )
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.xhermes itself — its parent is ~/ and its
        # directory name is ".xhermes", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: xhermes profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.xhermes itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.xhermes). "
            "Specify a different name: xhermes profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"xhermes.{old_name}"
