...
This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``XHERMES_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``XHERMES_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$XHERMES_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("XHERMES_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
