"""``xhermes logs`` — view and filter XHermes log files.
...
Usage examples::
    ...
    xhermes logs                    # last 50 lines of agent.log
    xhermes logs -f                 # follow agent.log in real time
    xhermes logs errors             # last 50 lines of errors.log
    xhermes logs gateway -n 100    # last 100 lines of gateway.log
    xhermes logs gui -f            # follow gui.log (dashboard/pty/ws)
    xhermes logs desktop -f        # follow desktop.log (Electron app boot/backend)
    xhermes logs --level WARNING    # only WARNING+ lines
    xhermes logs --session abc123   # filter by session ID substring
    xhermes logs --component tools  # only tool-related lines
    xhermes logs --since 1h         # lines from the last hour
    xhermes logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when XHermes runs — try 'xhermes chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'xhermes chat' to generate logs)")
