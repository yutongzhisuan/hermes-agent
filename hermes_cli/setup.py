"""
Interactive setup wizard for xHermes Agent.
...
Config files are stored in ~/.xhermes/ for easy access.
"""
...
_DOCS_BASE = "https://xhermes-agent.nousresearch.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during xhermes update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ XHermes Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure XHermes using environment variables or config commands:")
    print_info("  xhermes config set model.provider custom")
    print_info("  xhermes config set model.base_url http://localhost:8080/v1")
    print_info("  xhermes config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'xhermes setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``XHERMES_NONINTERACTIVE=1`` (see ``hermes_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("XHERMES_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``XHERMES_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'xhermes setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — XHermes cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    xhermes model            (pick any provider/model)")
        print_info("    xhermes setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'xhermes setup' to configure"))
    ...
    # Video generation — opt-in via `xhermes tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'xhermes setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'xhermes setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'xhermes tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'xhermes setup terminal'"))
    ...
    # Spotify (OAuth via xhermes auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'xhermes setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('xhermes setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('xhermes setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('xhermes setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('xhermes setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('xhermes setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('xhermes config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('xhermes config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('xhermes config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('xhermes', Colors.GREEN)}              Start chatting")
    print(f"   {color('xhermes gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('xhermes doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `xhermes tools` and `xhermes setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``xhermes model``)
    ...
    provider added to ``xhermes model`` is automatically available here.
    ...
    # Delegate to the shared xhermes model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: xhermes model")
    ...
    # on demand via `xhermes auth add`, `xhermes setup` vision, and
    # `xhermes setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``xhermes model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.xhermes/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the XHermes auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via xhermes setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'xhermes setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where XHermes runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `xhermes setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `xhermes setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `xhermes egress setup` then `xhermes egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `xhermes egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'xhermes-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $XHERMES_HOME/bin is never on PATH, so a bare
            # which() misses the uv XHermes installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'xhermes-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into XHERMES_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("XHERMES_MAX_ITERATIONS")
    ...
    print_info("  Run `xhermes setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # XHERMES_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.xhermes/logs/tool_calls.log (gateway only)")
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when XHERMES_HOME is a profile dir."""
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where XHermes delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects XHermes to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://xhermes-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://xhermes-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with XHermes from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'xhermes setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     xhermes config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: xhermes gateway install --system")
                print_info("  Or run in foreground:  xhermes gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   xhermes gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   xhermes gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `xhermes setup tools` and `xhermes tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this XHermes profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and XHermes.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure XHermes to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point XHermes at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point XHermes at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point XHermes at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point XHermes at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to XHermes equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing XHermes config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in XHermes.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ XHermes's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("XHermes can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: xhermes claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: xhermes claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing XHermes configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing XHermes config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in XHermes (use xhermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``xhermes setup --portal`` and ``xhermes portal``. This is the
    ...
    XHermes session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``xhermes setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``xhermes model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``xhermes portal`` always offers a model picker,
    ...
    print(color("│     ⚕ XHermes Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `xhermes portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `xhermes portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `xhermes portal`: %s", exc)
        print()
    ...
    print_info("  Run `xhermes portal info` to inspect routing.")
    print_info("  Run `xhermes` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      xhermes setup           — full or quick (auto-detected)
      xhermes setup model     — just model/provider
      xhermes setup tts       — just text-to-speech
      xhermes setup terminal  — just terminal backend
      xhermes setup gateway   — just messaging platforms
      xhermes setup tools     — just tool configuration
      xhermes setup telemetry — just local shared metrics
      xhermes setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ XHermes Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ xHermes Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your xHermes Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have XHermes configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'xhermes setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up XHermes?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'xhermes config edit'")
    ...
    # Tune later with `xhermes setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``xhermes setup <section>``
    or switch providers with ``xhermes model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'xhermes setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    xhermes setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  xhermes setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``xhermes tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``xhermes setup agent`` /
    ``xhermes config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `xhermes update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        xhermes tools")
        print_info("  Seed skills:         xhermes skills opt-in --sync")
        print_info("  Add MCP servers:     xhermes mcp add")
        print_info("  Enable plugins:      xhermes plugins")
        print_info("  Tune agent settings: xhermes setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`xhermes update` runs from re-injecting them. Opt back in any")
            print_info("time with `xhermes skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `xhermes tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `xhermes plugins list` / `xhermes plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `xhermes plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `xhermes mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `xhermes mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   xhermes tools")
    print_info("  Seed skills:         xhermes skills opt-in --sync")
    print_info("  Add MCP servers:     xhermes mcp add")
    print_info("  Tune agent settings: xhermes setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'xhermes setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect XHermes to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'xhermes setup gateway'.")
