"""Shell completion script generation for xhermes CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# xHermes Agent bash completion
# Add to ~/.bashrc:
#   eval "$(xhermes completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.xhermes/profiles"
...
complete -F _hermes_completion xhermes
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef xhermes
# xHermes Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.xhermes/profiles" ]]; then
        profiles+=($HOME/.xhermes/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'xhermes command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# xHermes Agent fish completion",
        "# Add to your config:",
        "#   xhermes completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.xhermes/profiles",
        "        for d in $HOME/.xhermes/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c xhermes -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c xhermes -f -s p -l profile"
