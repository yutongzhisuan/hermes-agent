...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``xhermes tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `xhermes tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``xhermes tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.xhermes/.env`` via ``save_env_value`` —
    the same store ``xhermes tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``xhermes tools post-setup <key>`` and the frontend tails the log via
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``xhermes computer-use permissions grant`` as a background action.
