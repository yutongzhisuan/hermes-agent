"""
xhermes fallback — manage the fallback provider chain.
...
Subcommands:
  xhermes fallback [list]   Show the current fallback chain (default when no subcommand)
  xhermes fallback add      Pick provider + model via the same picker as `xhermes model`,
                           then append the selection to the chain
  xhermes fallback remove   Pick an entry to delete from the chain
  xhermes fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.xhermes/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `xhermes model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  xhermes fallback add")
        print()
        return
    ...
    print("  Docs: https://xhermes-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `xhermes model`, then append the selection to the chain."""
    ...
    print("  `xhermes model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `xhermes fallback list` to view, or `xhermes fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``xhermes fallback [subcommand]``."""
