...
on Japanese locales, etc.).  XHermes's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. XHermes is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``XHERMES_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("XHERMES_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the XHermes-managed Git install directories so
    ...
    # already-running shells don't see the change — which means xhermes
    ...
    # install without XHermes-managed PortableGit).
    ...
    # stdin is re-configured for completeness; XHermes's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and XHermes's
    ``xhermes config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching XHermes (User env var in Windows Settings, or
    export in a PowerShell profile) and XHermes picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known XHermes-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\xhermes\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``xhermes`` from right after install) retain their old PATH.

    Any subprocess XHermes spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known XHermes-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "xhermes", "git", "cmd"),
        os.path.join(local_appdata, "xhermes", "git", "bin"),
        os.path.join(local_appdata, "xhermes", "git", "usr", "bin"),
        # XHermes venv Scripts directory — host of the xhermes.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes xhermes, but harmless to include.
        os.path.join(local_appdata, "xhermes", "xhermes-agent", "venv", "Scripts"),
