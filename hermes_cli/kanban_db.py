...
deployments where ``HERMES_HOME`` points outside ``~/.xhermes`` (e.g.
``/opt/xhermes``), ``<root>`` is ``HERMES_HOME``. Legacy env-var
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "xhermes-system"
