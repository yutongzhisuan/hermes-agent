"""
MCP Server Management CLI — ``xhermes mcp`` subcommand.

Implements ``xhermes mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.xhermes/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.xhermes/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``xhermes mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  xhermes mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  xhermes mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  xhermes mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: xhermes mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `xhermes mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  xhermes mcp add <name> --url <endpoint>')
        _info('  xhermes mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``xhermes mcp login`` and
    ``xhermes mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `xhermes mcp remove` + `xhermes mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `xhermes mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (XHermes desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `xhermes mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``xhermes mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``xhermes mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: xhermes mcp reauth <name>   |   xhermes mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── xhermes mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'xhermes mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``xhermes mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `xhermes plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("xhermes mcp                                    Open the catalog picker (default)")
        _info("xhermes mcp catalog                            List Nous-approved MCPs")
        _info("xhermes mcp install <name>                     Install a catalog MCP")
        _info("xhermes mcp serve                              Run as MCP server")
        _info("xhermes mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("xhermes mcp add <name> --command <cmd>         Add a stdio server")
        _info("xhermes mcp add <name> --preset <preset>       Add from a known preset")
        _info("xhermes mcp remove <name>                      Remove a server")
        _info("xhermes mcp list                               List configured servers")
        _info("xhermes mcp test <name>                        Test connection")
        _info("xhermes mcp configure <name>                   Toggle tools")
        _info("xhermes mcp login <name>                       Re-authenticate OAuth")
        _info("xhermes mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
