"""MCP picker — interactive `xhermes mcp picker` (also the default `xhermes mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``xhermes mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `xhermes plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new XHermes session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `xhermes mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: xhermes mcp install <name>    Picker: xhermes mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their XHermes is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer XHermes — run `xhermes update` "
...
def show_catalog() -> None:
    """`xhermes mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`xhermes mcp picker` (and default `xhermes mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`xhermes mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `xhermes mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
