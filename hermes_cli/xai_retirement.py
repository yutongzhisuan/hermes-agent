...
Pure logic: walks a XHermes config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `xhermes doctor` and a future `xhermes migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a XHermes config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a XHermes config and return retirement issues.
