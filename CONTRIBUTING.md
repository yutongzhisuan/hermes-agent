# Contributing to XHermes Agent

Thank you for contributing to XHermes Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want XHermes to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/xhermes-agent "<your terms>"
  gh search prs --repo NousResearch/xhermes-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/xhermes-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/xhermes-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — XHermes itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every XHermes install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `xhermes skills browse` (labeled "official") and install it with `xhermes skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `xhermes skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.xhermes/plugins/` (or via a pip entry point).
...
- Integrate with `xhermes memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. XHermes ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.xhermes/plugins/`, project `.xhermes/plugins/`, or a pip entry point) — see [Build a XHermes Plugin](https://xhermes-agent.nousresearch.com/docs/guides/build-a-xhermes-plugin)
...
Use this only if you intentionally do not want XHermes' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `xhermes` entrypoint from this venv; running the
...
export VIRTUAL_ENV="$HOME/.xhermes/venvs/xhermes-dev"
...
```bash
mkdir -p ~/.xhermes/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.xhermes/config.yaml
touch ~/.xhermes/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.xhermes/.env
```

### Run

```bash
# The standard installer already put `xhermes` on PATH.
xhermes doctor
xhermes chat -q "Hello"
```

If you used the manual clone fallback, run `./xhermes` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/xhermes" ~/.local/bin/xhermes
```
...
├── toolsets.py               # Tool groupings and presets (xhermes-cli, xhermes-telegram, etc.)
...
│
├── skills/                   # Bundled skills (copied to ~/.xhermes/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (xhermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.xhermes/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.xhermes/`)

| Path | Purpose |
|------|---------|
| `~/.xhermes/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.xhermes/.env` | API keys and secrets |
| `~/.xhermes/auth.json` | OAuth credentials (Nous Portal) |
| `~/.xhermes/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.xhermes/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.xhermes/state.db` | SQLite session database |
| `~/.xhermes/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.xhermes/cron/` | Scheduled job data |
| `~/.xhermes/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.xhermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_XHERMES_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.xhermes`:
...
The user may skip setup and keep loading the skill. XHermes only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `xhermes setup` or update `~/.xhermes/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native XHermes tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "XHermes Agent" is the secondary collaborator. If the contributor's commit shows "XHermes Agent" as author because they used XHermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing XHermes tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `xhermes --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

XHermes uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.xhermes/skins/<name>.yaml`:
...
XHermes runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the xhermes wrapper (it has a stdlib fallback
...
XHermes has terminal access. Security matters.
...
2. **Test manually**: Run `xhermes` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/xhermes-agent/issues)
- Include: OS, Python version, XHermes version (`xhermes version`), full error traceback
