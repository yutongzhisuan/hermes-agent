...
Regression guard for the Windows ``xhermes gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    r"C:\Users\me\xhermes\venv\Scripts\pythonw.exe -m hermes_cli.main gateway run",
    "python -m hermes_cli.main --profile work gateway run",
    "python -m hermes_cli.main gateway run --replace",
    "python -m hermes_cli/main.py gateway run",
    "python gateway/run.py",
    "xhermes-gateway.exe",
    "xhermes gateway",          # bare `xhermes gateway` defaults to run
    "xhermes gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "xhermes gateway --profile work run",
    "python -m hermes_cli.main gateway -p work run",
    "xhermes gateway --profile=work run",
    # a profile literally NAMED "gateway"
    "xhermes -p gateway gateway run",
    "python -m hermes_cli.main --profile gateway gateway run",
    # quoted Windows paths with spaces (shlex-aware tokenization)
    r'"C:\Program Files\XHermes\xhermes-gateway.exe"',
    r'"C:\Program Files\XHermes\gateway\run.py" run',
