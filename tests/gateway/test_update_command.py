...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", tmp_path):
            ...
            # project_root will be the real xhermes-agent dir (which HAS .git).
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", hermes_home), \
             ...
             patch("shutil.which", side_effect=lambda x: "/usr/bin/xhermes" if x == "xhermes" else "/usr/bin/setsid"), \
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting XHermes update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        # Later steps may legitimately return success ("Starting XHermes
