...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``XHERMES_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "XHERMES_SESSION_CHAT_ID": env.get("XHERMES_SESSION_CHAT_ID"),
        "XHERMES_SESSION_THREAD_ID": env.get("XHERMES_SESSION_THREAD_ID"),
        "XHERMES_SESSION_KEY": env.get("XHERMES_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("XHERMES_SESSION_CHAT_ID", "XHERMES_SESSION_THREAD_ID", "XHERMES_SESSION_KEY"):
    ...
    assert captured["bound"]["XHERMES_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["XHERMES_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``XHERMES_SESSION_*``
