...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate XHERMES_HOME under tmp_path."""
    monkeypatch.setenv("XHERMES_HOME", str(tmp_path / ".xhermes"))
    (tmp_path / ".xhermes").mkdir()

    # Force the constants/image cache helpers to re-read XHERMES_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under XHERMES_HOME — gateway cleanup
