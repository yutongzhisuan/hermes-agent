...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp XHERMES_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "nous-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "nous", "model": "xhermes-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "nous",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "xhermes-4", "https://inference-api.nousresearch.com/v1", "max_tokens"),
        ],
    )
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``xhermes auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XHERMES_HOME", str(hermes_home))
        monkeypatch.setenv("XHERMES_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``xhermes model``, no per-task override is
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through XHermes auth / credential pool."""
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so XHermes owns the retry/timeout budget (issue #54465). The SDK
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized XHermes reasoning.

    MoA slot reasoning is carried through call_llm as a XHermes
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NousResearch/xhermes-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NousResearch/xhermes-agent/issues/18478
    """
