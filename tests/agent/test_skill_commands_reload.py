...
def hermes_home(monkeypatch):
    """Isolate XHERMES_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``XHERMES_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="xhermes-reload-skills-")
    monkeypatch.setenv("XHERMES_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "XHERMES_HOME", home, raising=False)
