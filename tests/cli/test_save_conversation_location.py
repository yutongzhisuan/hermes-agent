...
file to be discoverable via ``xhermes sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.xhermes/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.xhermes/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the XHERMES_HOME fixture
    ...
    # File MUST be under ~/.xhermes/sessions/saved/
    ...
    assert "xhermes --resume 20260101_120000_abc123" in out, out
