...
``os.environ["XHERMES_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``xhermes --yolo`` (process-start env), ``XHERMES_YOLO_MODE=1``,
...
def _clear_approval_state(monkeypatch):
    ...
    # XHermes-driven test runs may inherit XHERMES_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``xhermes --yolo`` sets ``XHERMES_YOLO_MODE`` before tool imports, so
