...
profile's XHERMES_HOME and EXECUTE under it.
...
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active XHERMES_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever XHERMES_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    XHERMES_HOME is a profile under it (``<root>/profiles/<name>``)."""
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile XHERMES_HOME (<root>/profiles/<name>), the cron store
    ...
    # cron/jobs.py computes XHERMES_DIR from get_hermes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.XHERMES_DIR.resolve() == profile_home.resolve()
        assert (
