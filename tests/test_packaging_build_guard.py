...
def _build_artifact(
    ...
    *,
    ...
    headless_wheel_build: bool = False,
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for xhermes-agent is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "xhermes_agent-*.tar.gz"), ("wheel", "xhermes_agent-*.whl")],
)
...
def test_artifact_build_allows_headless_wheel_build_marker(kind, artifact_glob, tmp_path):
    result = _build_artifact(kind, tmp_path, headless_wheel_build=True)
