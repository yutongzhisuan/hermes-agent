"""Harness: dashboard opt-in via XHERMES_DASHBOARD.

Today (tini): dashboard starts once when XHERMES_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    """Without XHERMES_DASHBOARD, no dashboard process should be running."""
    ...
    assert r.returncode != 0, (
        "Dashboard should not be running without XHERMES_DASHBOARD"
    )
...
# whenever `XHERMES_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``XHERMES_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "XHERMES_DASHBOARD=1",
        "XHERMES_DASHBOARD_HOST=0.0.0.0",
        "XHERMES_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"XHERMES_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and XHERMES_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``XHERMES_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
