...
package-lock.json. Without XHERMES_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV XHERMES_TUI_DIR=/opt/xhermes/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the xhermes user, return stdout."""
    inner = (
        "source /opt/xhermes/.venv/bin/activate && "
        "cd /opt/xhermes && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the xhermes user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "xhermes", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """XHERMES_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$XHERMES_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/xhermes/ui-tui", (
        f"XHERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/xhermes/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    # With XHERMES_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
