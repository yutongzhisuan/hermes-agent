...
"""
    ...
    assert "Tyler Williams > Active Projects: XHermes Agent" in entries
...
def test_readable_config_keeps_every_pre_existing_key(tmp_path: Path):
    ...
    assert merged["model"] == "xhermes-4-405b"
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.xhermes/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: xhermes-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "xhermes-4-405b" in real.read_text(encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.xhermes/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``XHermes``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "XHermes prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told XHermes to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "XHermes config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "XHermes uses tools well"
    # All-lowercase matches → lowercase ``xhermes``; this preserves the
    # real filesystem path ``~/.xhermes`` (XHermes home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "xhermes should always respond concisely"
