...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked XHERMES_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout


def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    assert 'chown_hermes_tree "$XHERMES_HOME/$sub"' in stage2_text
    assert 'chown_hermes_tree "$XHERMES_HOME/profiles"' in stage2_text
    assert 'chown_hermes_tree "$XHERMES_HOME/cron"' in stage2_text
    assert 'chown -R xhermes:xhermes "$XHERMES_HOME/$sub"' not in stage2_text
    assert 'chown -R xhermes:xhermes "$XHERMES_HOME/profiles"' not in stage2_text
    assert 'chown -R xhermes:xhermes "$XHERMES_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_hermes_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$XHERMES_HOME"' in stage2_text
...
def test_stage2_skips_recursive_repairs_when_tree_is_already_owned(
    ...
    assert 'if [ -e "$XHERMES_HOME/$sub" ] && tree_has_non_hermes_owner "$XHERMES_HOME/$sub"; then' in stage2_text
    assert 'if [ -d "$XHERMES_HOME/profiles" ] && tree_has_non_hermes_owner "$XHERMES_HOME/profiles"; then' in stage2_text
    # Sibling every-boot chown blocks carry the same warm-boot gate.
    assert 'if [ -d "$XHERMES_HOME/cron" ] && tree_has_non_hermes_owner "$XHERMES_HOME/cron"; then' in stage2_text
    assert 'if [ -d "$XHERMES_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$XHERMES_HOME/platforms/pairing"; then' in stage2_text
    assert 'if [ -d "$XHERMES_HOME/pairing" ] && tree_has_non_hermes_owner "$XHERMES_HOME/pairing"; then' in stage2_text
