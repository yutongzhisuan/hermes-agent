...
TTS provider tools must resolve API keys from ``~/.xhermes/.env`` (via
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.xhermes/.env`` carries the
...
class TestRegressionGuard:
    ...
    ``hermes_cli.config.load_env`` to simulate ``~/.xhermes/.env`` carrying the
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.xhermes/.env carrying the key (load_env returns the dict
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.xhermes/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
