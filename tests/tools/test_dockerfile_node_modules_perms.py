...
Older images made /opt/xhermes/ui-tui and /opt/xhermes/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/xhermes/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R xhermes:xhermes /opt/xhermes/ui-tui" not in text
    assert "chown -R xhermes:xhermes /opt/xhermes/node_modules" not in text
