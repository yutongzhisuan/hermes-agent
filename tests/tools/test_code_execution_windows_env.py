...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``XHERMES_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # XHERMES_* handling (#27303): only the operational allowlist passes;
        # every other XHERMES_* is dropped (the broad prefix was removed).
        "XHERMES_HOME": "/home/alice/.xhermes",        # allowlisted → kept
        "XHERMES_PROFILE": "default",                 # allowlisted → kept
        "XHERMES_INTERACTIVE": "1",                   # not allowlisted → dropped
        "XHERMES_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "XHERMES_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
