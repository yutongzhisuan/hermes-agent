...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``xhermes tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `xhermes tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``xhermes tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``xhermes auth`` sign-in) rather than an env-var key.
    ...
    def test_search_uses_oauth_bearer_token_and_base_url(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("XAI_API_KEY", raising=False)
        monkeypatch.setenv("XHERMES_HOME", str(tmp_path))
        monkeypatch.setenv("XHERMES_XAI_BASE_URL", "https://proxy.x.ai/v1/")
