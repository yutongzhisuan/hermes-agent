...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.xhermes/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.xhermes/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XHERMES_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.xhermes")

        assert mounts[0]["container_path"] == "/home/user/.xhermes/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.xhermes/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.xhermes/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape XHERMES_HOME."""
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of XHERMES_HOME must be allowed."""
        hermes_home = tmp_path / ".xhermes"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside XHERMES_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".xhermes"
        hermes_home.mkdir()
        monkeypatch.setenv("XHERMES_HOME", str(hermes_home))
        ...
        # The resolved path escapes XHERMES_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape XHERMES_HOME."""
        hermes_home = tmp_path / ".xhermes"
        hermes_home.mkdir()
        monkeypatch.setenv("XHERMES_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside XHERMES_HOME via config must still mount."""
        hermes_home = tmp_path / ".xhermes"
        hermes_home.mkdir()
        monkeypatch.setenv("XHERMES_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.xhermes/cache/documents" in paths
        assert "/root/.xhermes/cache/audio" in paths
        assert "/root/.xhermes/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.xhermes/cache/documents" in container_paths
        assert "/root/.xhermes/cache/images" in container_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``XHERMES_HOME/images``, not
        ...
        assert "/root/.xhermes/images" in by_container
        assert by_container["/root/.xhermes/images"] == str(hermes_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.xhermes/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.xhermes/cache/images/generated.png"
        )
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — XHERMES_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping XHERMES_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"XHERMES_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.xhermes/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.xhermes/google_token.json" in paths
        assert "/root/.xhermes/.env" not in paths
