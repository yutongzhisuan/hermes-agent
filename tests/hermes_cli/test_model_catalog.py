...
def isolated_home(tmp_path, monkeypatch):
    """Isolate XHERMES_HOME + reset any module-level catalog cache per test."""
...
class TestFallbackChain:
    ...
    PRIMARY = "https://xhermes-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/xhermes-agent"
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``xhermes model`` CLI — not the live
        ...
        # ``XHERMES_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` XHERMES_HOME directly instead.
        import importlib
        from hermes_cli import model_catalog
        ...
        try:
            ...
            active_home = Path(os.environ["XHERMES_HOME"])
