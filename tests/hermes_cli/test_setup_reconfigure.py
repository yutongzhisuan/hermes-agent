...
- Bare `xhermes setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `xhermes setup --quick` runs the narrower "fill in missing items" flow.
- `xhermes setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `xhermes setup` on an existing install = full reconfigure wizard."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xhermes", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
