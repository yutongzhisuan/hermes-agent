...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".xhermes"
    second = tmp_path / "b" / ".xhermes"

    monkeypatch.setenv("XHERMES_HOME", str(first))
    cfg.ensure_hermes_home()

    # Profile switch: XHERMES_HOME moves → the new path is ensured too.
    monkeypatch.setenv("XHERMES_HOME", str(second))
    cfg.ensure_hermes_home()
