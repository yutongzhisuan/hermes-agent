...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        xhermes = "hermes_cli.main:main"
        xhermes-agent = "run_agent:main"
        xhermes-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("xhermes", "xhermes-agent", "xhermes-acp"):
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"xhermes.exe", "xhermes-agent.exe", "xhermes-acp.exe"} <= names
        assert "xhermes-gateway.exe" in names
