...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("XHERMES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """XHermes' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` XHermes' DB/metadata/logs on task completion.
    """
...
# spawned with `xhermes -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's XHERMES_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's XHERMES_HOME."""

    def _set_home(self, monkeypatch, tmp_path, hermes_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XHERMES_HOME", str(hermes_home))
        monkeypatch.delenv("XHERMES_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.xhermes/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.xhermes/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.xhermes/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # XHERMES_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile XHERMES_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # XHERMES_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is XHERMES_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["XHERMES_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["XHERMES_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["XHERMES_KANBAN_TASK"] == "t_dispatch_env"
        assert env["XHERMES_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "XHERMES_SESSION_SOURCE":
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'xhermes kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
#
# Workers spawned by the dispatcher must use a `xhermes` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["xhermes", ...]`
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    Pins the correct module name (NOT `xhermes` — there is no top-level
    `xhermes` package). Regression for #23198: the original PR shipped
    `python -m xhermes` which fails with `No module named xhermes` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.delenv("XHERMES_BIN", raising=False)
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("XHERMES_BIN", None)
    ...
    assert "XHermes Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
