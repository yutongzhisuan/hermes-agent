"""Tests for IRC gateway configuration via `xhermes setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh XHermes install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new XHermes install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#xhermes")
            monkeypatch.setenv("IRC_NICKNAME", "xhermes-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `xhermes setup gateway` experience with IRC present."""
