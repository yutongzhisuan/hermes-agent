...
Before the fix, ``xhermes whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``xhermes gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    xhermes = home / ".xhermes"
    xhermes.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("XHERMES_HOME", str(xhermes))
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``xhermes whatsapp`` with an existing paired session and
