...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("XHERMES_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_hermes_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp XHERMES_HOME."""
    hh = tmp_path / "xhermes-home"
    hh.mkdir()
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no XHERMES_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("XHERMES_OPTIONAL_MCPS", raising=False)
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything XHermes fetches/launches is pinned
        to an exact version.
