...
($INSTALL_DIR), not into the shared $XHERMES_HOME.

Background (shared-$XHERMES_HOME bug)
------------------------------------
$XHERMES_HOME is a data directory users frequently bind-mount into a Docker
...
$XHERMES_HOME the two installs would fight over one slot — and the container,
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$XHERMES_HOME/\.install_method"', text), (
        "install.sh must not stamp $XHERMES_HOME/.install_method — that data "
