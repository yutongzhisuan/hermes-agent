# OpenAI-Compatible API Server for XHermes Agent
...
SSE streaming. By exposing this endpoint, xhermes-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  xhermes-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.xhermes/.env`
2. Run `xhermes gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with xhermes-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns xhermes-agent as a model) |
...
Authorization: Bearer xhermes-api-key-here
...
But xhermes-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "xhermes-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured xhermes-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of xhermes-agent's core prompt
...
Once implemented, xhermes-agent works as a drop-in backend for:
