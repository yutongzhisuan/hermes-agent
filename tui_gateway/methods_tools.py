...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.xhermes/.env`` into the gateway process via
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``xhermes journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``xhermes plugins`` / the dashboard, so the three surfaces
