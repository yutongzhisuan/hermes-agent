 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.xhermes/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.xhermes', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (XHERMES_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.XHERMES_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.xhermes', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.XHERMES_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.xhermes', 'document_cache');
const AUDIO_CACHE_DIR = process.env.XHERMES_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.xhermes', 'audio_cache');
...
// `xhermes update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *XHermes Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls XHermes itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['XHermes Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into XHermes and trigger
      ...
      // Ignore XHermes' own reply messages in self-chat mode to avoid loops.
