...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{XHERMES_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``XHERMES_HOME`` is a **durable** store — on XHermes
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting XHERMES_HOME."""
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable XHERMES_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (XHERMES_HOME is a
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable XHERMES_HOME volume is
