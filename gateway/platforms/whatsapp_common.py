...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *xHermes Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to XHERMES_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/xhermes), this function
    mirrors the bridge source to a writable XHERMES_HOME location and returns that
    ...
    # Try XHERMES_HOME location first
    ...
    # Install dir is read-only, mirror to XHERMES_HOME if needed
    if hermes_home_bridge.exists():
        return hermes_home_bridge

    # Mirror the bridge source to XHERMES_HOME
    try:
