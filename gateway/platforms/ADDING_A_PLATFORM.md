...
There are two ways to add a platform to the XHermes gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.xhermes/plugins/` (or under `plugins/platforms/`
...
**zero changes to core XHermes code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `xhermes gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the XHermes core.
...
And add it to the `xhermes-gateway` composite:

```python
"xhermes-gateway": {
    "includes": [..., "xhermes-your-platform"]
}
```
