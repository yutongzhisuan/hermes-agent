"""
XHermes MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (XHermes-specific extra)

Usage:
    xhermes mcp serve
    xhermes mcp serve --verbose
...
logger = logging.getLogger("xhermes.mcp_serve")
...
class EventBridge:
    ...
    This is the XHermes equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the XHermes MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "xhermes",
        instructions=(
            "xHermes Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the XHermes MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
