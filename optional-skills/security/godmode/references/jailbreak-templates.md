...
## 5. GODMODE FAST — XHermes 4 405B (Zero Refusal)

**Model:** `nousresearch/xhermes-4-405b`
**Strategy:** XHermes models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with XHermes Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.xhermes/config.yaml`:
