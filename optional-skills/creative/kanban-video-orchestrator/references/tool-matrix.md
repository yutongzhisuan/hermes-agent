...
Maps each role archetype to the XHermes skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public xhermes-agent
...
## XHermes skills relevant to video production

### Visual / rendering skills (`xhermes-agent/skills/creative/`)
...
### Design / pre-production skills (`xhermes-agent/skills/creative/`)
...
### Audio / media skills (`xhermes-agent/skills/creative/` + `skills/media/`)
...
These are **not** XHermes skills but external CLIs / APIs that profiles invoke.
...
## Built-in XHermes tools for media review

These are native XHermes tools — not invoked via terminal but through their own
...
key is present in `${XHERMES_HOME:-~/.xhermes}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every XHermes profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${XHERMES_HOME:-~/.xhermes}/.env` → environment variable.
...
When a new XHermes-public video skill ships:
