...
key is present in `${HERMES_HOME:-~/.xhermes}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${HERMES_HOME:-~/.xhermes}/.env` → environment variable.
