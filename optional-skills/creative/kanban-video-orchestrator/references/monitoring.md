...
xhermes kanban watch --tenant <project-slug>

# Snapshot of the board
xhermes kanban list --tenant <project-slug>
xhermes kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
xhermes kanban stats --tenant <project-slug>

# Visual dashboard (browser)
xhermes dashboard

# Inspect a specific task (includes comments + events)
xhermes kanban show <task-id>

# Follow a single task's event stream
xhermes kanban tail <task-id>
```

Verify available subcommands with `xhermes kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `xhermes kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `xhermes kanban block <id>` then `xhermes kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `xhermes kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `xhermes kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `xhermes profile list` (profile exists?), `xhermes status` (gateway/dispatcher up?), and verify tenant. |
...
xhermes kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
xhermes kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(xhermes kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
xhermes kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
xhermes kanban block <task-id>
xhermes kanban archive <task-id>

# Diagnose what happened
xhermes kanban show <task-id>      # task body, comments, recent events
xhermes kanban tail <task-id>      # follow the live event stream
xhermes kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `xhermes kanban create` for the director
...
while true; do
    clear
    xhermes kanban list --tenant <slug>
    echo "---"
    xhermes kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `xhermes kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `xhermes profile list` and start
