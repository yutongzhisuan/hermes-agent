...
from extend.task_relay.worker.task_executor import TaskBackend, TaskCancelEvent, TaskRunPayload
...
async def _acp_run(state: AcpRpcState, params: dict[str, Any]) -> dict[str, Any]:
    ...
    async def _noop_checkpoint(**_kwargs: Any) -> None:
        return None

    async def _execute() -> dict[str, Any]:
        payload = await state.backend.run(
            run, _noop_progress, _noop_checkpoint, cancel_event
        )
        return {
...
def create_acp_rpc_app(
    *,
    ...
    if backend is None:
        ...
        backend = AcpTaskBackend(progress_interval_seconds=progress_interval_seconds)
...
async def serve_acp_rpc(
    *,
    ...
    app = create_acp_rpc_app(progress_interval_seconds=progress_interval_seconds)
