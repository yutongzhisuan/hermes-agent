 ...
 * packaged binary) with a real `xhermes serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → xhermes serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with XHermes title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('XHermes')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → xhermes serve is spawned → WS connects → config
