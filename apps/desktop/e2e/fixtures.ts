 ...
 *
 * Key env vars:
 *  - XHERMES_HOME → sandbox xhermes-home (isolated config/sessions)
 *  - XHERMES_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - XHERMES_DESKTOP_IGNORE_EXISTING=1 → don't pick up `xhermes` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - XHERMES_DESKTOP_XHERMES_ROOT → REPO_ROOT (dev checkout resolution)
 *  - XHERMES_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    XHERMES_DESKTOP_APP_NAME: `HermesE2E-${Date.now()}`,
    ...
    XHERMES_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated XHERMES_HOME + userData
 * @param env      - the process environment (already has XHERMES_HOME etc.)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via XHERMES_DESKTOP_BOOT_FAKE_ERROR
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { XHERMES_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to XHermes backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
 ...
 * XHERMES_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    XHERMES_DESKTOP_BOOT_FAKE: '1',
    XHERMES_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + xhermes-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).XHERMES_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).XHERMES_DESKTOP_XHERMES
  delete (env as Record<string, string | undefined>).XHERMES_DESKTOP_XHERMES_ROOT
