...
// no longer bundles the XHermes Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'XHermes.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'XHermes'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'XHermes.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'XHermes'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `XHermes-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `XHermes-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `XHermes-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'XHermes-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  const hermesHome = path.join(sandbox, 'xhermes-home')
...
//   - The XHermes Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'xhermes-agent', 'hermes_cli', 'main.py')
