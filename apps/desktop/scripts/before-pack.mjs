 ...
 * product name (`XHermes`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/XHermes'
 *
 * This is a hard failure with no obvious cause for the user — `xhermes desktop`
 ...
 * (the mac-unpacked XHermes.app bundle) and Windows (win-unpacked), so we
 ...
 * XHermes.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `xhermes desktop --build-only` (hermes_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'XHermes.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'XHermes'}.exe`
