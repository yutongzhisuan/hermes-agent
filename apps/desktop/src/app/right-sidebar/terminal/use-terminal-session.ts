...
const XHERMES_PATHS_MIME = 'application/x-xhermes-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(XHERMES_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(XHERMES_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (xhermes --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[xhermes-terminal] WebGL unavailable; falling back to DOM', err)
      }
