 ...
 * billing-specific toast — never the generic "XHermes error" — with a smart CTA
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        // A runtime skin switch (XHermes activating an authored skin, or `/skin`
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'XHermes reported an error',
        ...
        // native OS notification when XHermes is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'XHermes reported an error'
