...
import { disconnectOAuthProvider, listOAuthProviders } from '@/xhermes'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/xhermes'
...
//      `xhermes model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // XHermes can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider XHermes can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // XHermes never deletes creds another tool owns behind a silent API call.
