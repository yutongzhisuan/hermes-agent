...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting XHermes...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting XHermes…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to XHermes gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "XHermes couldn't start"
