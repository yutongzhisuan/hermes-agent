...
//
// When someone is in a hands-free "Hey XHermes" voice chat, the natural way to
...
// phrase (optionally addressed to XHermes), so a real turn that merely contains
...
// Optional address prefixes so "xhermes stop" / "ok stop" / "hey xhermes, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey xhermes', 'hey xhermes,', 'xhermes', 'xhermes,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("xhermes") is not a stop command on its own.
      continue
    }
 ...
 * to XHermes). Returns false for anything that merely contains "stop" as part of
