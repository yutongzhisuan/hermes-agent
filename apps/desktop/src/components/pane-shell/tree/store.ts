...
const STORAGE_KEY = 'xhermes.desktop.layoutTree.v2'

writeKey('xhermes.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('xhermes.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('xhermes.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'xhermes.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'xhermes.desktop.userPlacedPanes.v1'
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__XHERMES_LAYOUT_TREE__ = {
    close: closeTreePane,
