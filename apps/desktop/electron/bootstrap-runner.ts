 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of XHermes Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.xhermes-bootstrap-complete'), 'utf8')
...
// checkout under ~/.xhermes/xhermes-agent. Used as a last-resort fallback when
...
function installedAgentInstallScript(hermesHome) {
  ...
  const candidate = path.join(hermesHome, 'xhermes-agent', 'scripts', installScriptName())
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/xhermes-agent/${ref}/scripts/${scriptName}`
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass XHERMES_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          XHERMES_HOME: hermesHome || process.env.XHERMES_HOME || ''
        }
      })
    )
...
function buildPosixPinArgs({ installStamp, activeRoot, hermesHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--xhermes-home', hermesHome]
