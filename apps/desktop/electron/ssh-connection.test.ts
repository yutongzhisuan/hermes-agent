...
test('redactSecrets scrubs Authorization and X-XHermes-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-XHermes-Session-Token: hdr_888'), /X-XHermes-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-XHermes-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('xhermes', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v xhermes', 15000)
  assert.equal(args[args.length - 1], 'command -v xhermes')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `xhermes-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `xhermes-ssh-test-${process.pid}-${Date.now()}`)
