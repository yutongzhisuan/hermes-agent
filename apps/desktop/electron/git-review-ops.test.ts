...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'xhermes-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'xhermes-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'XHermes Test'], { cwd: dir })
