/**
 * windows-xhermes-path.ts
 *
 * Pure, dependency-injected pieces of Windows `xhermes` resolution pulled out
 ...
 *      FIRST, so an extensionless Git-Bash `xhermes` shim shadowed the real
 *      xhermes.cmd/xhermes.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\xhermes.exe (the console-script shim, written at the END
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `xhermes`) must
 * not shadow `xhermes.cmd`/`xhermes.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv xhermes shim, bootstrap-complete
 * marker) — gating on just the xhermes.exe console-script shim alone is the
...
/**
 * If `command` is a Windows venv `xhermes`/`xhermes.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/xhermes(.exe)`), resolve it to the underlying
 ...
 * venv whose update died mid-`pip install` still has python.exe + xhermes.exe
 ...
 *
 * Returns null when `command` is not a venv xhermes shim, the underlying
...
export function resolveVenvHermesCommand(
  ...
  if (!/^xhermes(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv XHermes at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing XHermes Python at ${python}`,
    command: python,
