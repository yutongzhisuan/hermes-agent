...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/xhermes', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/xhermes/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/xhermes'), 'https://gw.example.com/xhermes/auth/native/refresh')
})
