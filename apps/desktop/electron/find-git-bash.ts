 ...
 *   1. XHERMES_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\xhermes\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect XHERMES_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.XHERMES_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'xhermes', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'xhermes', 'git', 'usr', 'bin', 'bash.exe'))
  }
