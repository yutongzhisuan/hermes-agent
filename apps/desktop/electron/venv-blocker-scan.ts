...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another XHermes process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the XHermes installation is free.\n' +
    '\n' +
    'Close other XHermes windows and terminals, then retry.  If the problem\n' +
    'persists, run `xhermes update` in a terminal for detailed diagnostics.'
  )
}
