...
//!
//! Driven when the installer is launched as `XHermes-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old XHermes desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `xhermes update`
//!      or repair bootstrap can race locked files),
//!   2. run `xhermes update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hermes_cli/main.py),
//!   3. run `xhermes desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `xhermes update` already handles macOS/Linux (git/pip).
...
/// `xhermes update` exit code meaning "another xhermes process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// hermes_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `xhermes update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `xhermes update` claims
...
/// overwriting — the pre-fix clobber is what let a dashboard `xhermes update`
...
/// owner that is itself and aborts ("Another XHermes update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `xhermes update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("xhermes-agent");
    ...
    // updater — most often a dashboard-spawned `xhermes update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another XHermes update is already running (PID {}, started {} ago). \
    ...
    let xhermes = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the xhermes CLI under {}. Is XHermes installed? \
    ...
    // async on Windows. If it still holds the venv shim, `xhermes update`
    ...
    // ---- stage 2: xhermes update -----------------------------------------
    // Pass --branch so `xhermes update` targets the branch this installer was
    ...
    // without --branch, `xhermes update` switches the checkout to `main` (a
    ...
    // --force skips `xhermes update`'s Windows running-exe guard (which would
    ...
    // time `xhermes update` runs there is no legitimate xhermes.exe to protect,
    // and the guard would only produce a false "XHermes is still running" stop.
    //
    ...
    // probe only checks the xhermes.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all XHermes windows" message.
    ...
    // Retry-once for the update-boundary crash. `xhermes update` lazily imports
    ...
    // second `xhermes update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close XHermes" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "XHermes is still running. Close all XHermes windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "xhermes update failed (exit {:?}). See {} for details.",
                other,
    ...
    // ---- stage 3: xhermes desktop --build-only ----------------------------
    // `xhermes update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `xhermes update` already does above, and `xhermes update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `xhermes desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch XHermes manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for XHermes to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend xhermes.exe (or the desktop XHermes.exe
            ...
            // force-kill every XHermes.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] XHermes still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("XHermes.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("XHermes.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `xhermes.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `xhermes.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\xhermes.exe update`. And a
...
/// xhermes.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named xhermes.exe.)
...
/// Spawn `xhermes <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv xhermes shim under an install root, regardless of existence.
fn venv_hermes(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("xhermes.exe")
    } else {
        install_root.join("venv").join("bin").join("xhermes")
    }
}

/// Resolve the xhermes CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `xhermes` on PATH.
fn resolve_hermes(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "xhermes.exe" } else { "xhermes" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `xhermes update` child claims that SAME lock (hermes_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("XHermes is still running")
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no XHermes.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.xhermes-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.xhermes-update-old", target_app.display()));
...
mod tests {
    ...
    fn venv_hermes_is_under_install_root() {
        let root = Path::new("/x/xhermes-agent");
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/xhermes-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".xhermes-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `xhermes update` and install-mode bootstrap mutate one tree
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/XHermes.app"]),
            Some(PathBuf::from("/Applications/XHermes.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("XHermes.app");
        let tmp = base.join("XHermes.app.xhermes-update-new");
        let old = base.join("XHermes.app.xhermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("XHermes.app");
        let tmp = base.join("XHermes.app.xhermes-update-new"); // intentionally absent
        let old = base.join("XHermes.app.xhermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("XHermes.app");
        let tmp = base.join("XHermes.app.xhermes-update-new"); // absent
        let old = base.join("XHermes.app.xhermes-update-old");
        write_marker(&target, "OLD");
