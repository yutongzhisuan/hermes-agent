...
pub fn hermes_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\xhermes — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("xhermes");
        }
    }

    // macOS + Linux + fallback: ~/.xhermes (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".xhermes");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".xhermes")
}
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[xhermes-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
