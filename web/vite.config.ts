...
const BACKEND = process.env.XHERMES_DASHBOARD_URL ?? "http://127.0.0.1:9119";
 ...
 * load, scrapes the `window.__XHERMES_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__XHERMES_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__XHERMES_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__XHERMES_SESSION_TOKEN__="${match[1]}";` +
              `window.__XHERMES_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[xhermes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`xhermes dashboard\` or set XHERMES_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@xhermes/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
