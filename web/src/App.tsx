...
const SIDEBAR_COLLAPSED_KEY = "xhermes-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="xhermes-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="xhermes-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'xhermes update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "xhermes update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'xhermes update' (${cmd}) and restart the gateway when it finishes.`
    );
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the XHermes gateway process. Connected channels and active sessions will reconnect afterward."
      }
